...
| Earlier measurements proved the environment was already near its throughput ceiling. | The selected randomized-wave run completed 10,485,760 agent steps at 56,575.81 SPS. The final optimized same-shape candidate reached 79,848.92 SPS. Under an identical renderer-free simulator protocol, K=128 randomized and fixed medians were 56,314.635 and 57,135.018 SPS. | **False.** Complete randomized training and controlled benchmarks establish useful throughput, without locating a saturation point. The simulator remains CPU-bound and its water kernels still dominate sampled CPU time. |
...
| Every reset reused the same wave field | Production selects a coherent race-start state from a finite pool of complete cartridge boots. Checked 4 KiB deltas reconstruct donor RDRAM while each environment keeps its own native stack and context. Fixed mode remains available for parity regressions. | All 128 seed-902 donors matched reconstructed owners across reset state and two 32-decision scripts; complete fields were unique, exact replay passed, and both seed-2902 and post-selection seed-16902 pools had zero overlap with training. | Pool size is finite and supported only for the scoped Sunny Beach task. |
...
The 2026-08-24 performance audit used the same standard Puffer agent SPS reported for every first-party environment. A scheduler-only A/B at 128 agents and 16 workers measured 59,158 SPS with static assignment and a 75,127.85 SPS four-run median with Wave Race-specific dynamic assignment, a 27.00% gain. The checked-in one-buffer shape then reached 75,832.65 SPS. Correct GB10 targeting plus `-fno-math-errno` reproduced a further 4.50% mean gain while retaining `-ffp-contract=off`. The final production candidate reached 79,848.92 SPS over 2,097,152 steps. Stack `MADV_NOHUGEPAGE` saved 254.9 MiB of controlled peak RSS. The rejected full-RDRAM copy reset was slower and consumed more memory.
...
The selected 438,272-byte, 109,568-parameter checkpoint is `/home/spark-advantage/wr64-results/obs57-authentic-waves/checkpoints/waverace64/obs57-authwave-s903-final/0000000010485760.bin`, SHA-256 `91217a7eb1ff5f4d553b678c206c5acaa727461bcf45cfd4f0266f7c2e0f62bf`. Fresh deterministic CUDA processes completed 128/128 official three-lap episodes on seed 2902 and 128/128 on post-selection, training-disjoint seed 16902. Separate one-environment runs completed exact odd-stride permutations of all 128 variants at 128/128 on both pools. Stochastic CUDA completed 510/512 on each pool. Each stochastic run had one generic failure and one disqualification; all runs had zero safety timeout and zero environment fault. Seed-16902 deterministic, stochastic, and strict-permutation log hashes are `32d52753f082d7f670baa4284fb702a59582ffc58d5c7c27702614d676732163`, `a444de2d01c74c8774a19b4f929788f38ac65306ebbccc0390fbf28550966201`, and `3929c1e30c8fbdb0b577575aec71ef4230851e5d568103a5b29620c6f92ffc42`. A separate CPU process loaded exactly 109,568 floats and completed one seed-16902 race in 883 decisions with 46 checkpoints, one miss, and zero failure, disqualification, timeout, or fault. Its retained log hash is `ad79bb3f262092c30d893ac4bad1f441c54855a3c1d402e840eb439117831f16`.

The current full-race MP4 is `/home/spark-advantage/wr64-results/obs57-authentic-waves/video/waverace64-puffer-s903-disjoint16902-full-race.mp4`, 21,310,915 bytes, SHA-256 `a89e81fbb42be0212c3c175b4621efe3fde957e753f59394989e8ab22f6a5835`. It is H.264 High/yuv420p at 960 by 540 and 60 frames/s. It contains 5,294 time-zero-through-terminal source frames plus a 240-frame frozen finish, 5,534 decoded frames total, and lasts 92.23 s. Full decode and independent start, midpoint, and terminal frame inspection passed; the official finish is 87.898 s with no episode-two contamination. Capture, encode, and full-decode log hashes are `4ff46be7f4deeb0319463dea455c2d29a3c05a4737a1530db7fc4861bd340397`, `583d73abff232f75b1b0415ffca7e9822071ae6f5baa3cfdea24a790c25b27da`, and `a5b633665078c0c6bfa4a2fc49fa2162a4df6e9684a3a7f386f9cb8a3909204f`.
...
| --- | --- |
| Randomized-wave OBS57 PufferLib 5.0 performance | **MEASURED.** Selected full trainer run sustained 56,575.81 Puffer SPS. The final optimized same-shape candidate reached 79,848.92 SPS. No throughput ceiling is claimed. |
| Randomized-wave OBS57 learning | **PASS.** Selected seed 903 completed 128/128 deterministic races on held-out seed 2902 and 128/128 on post-selection, training-disjoint seed 16902; stochastic was 510/512 on each. |
| Native race timing | **PASS.** Selected video finished in native time 87,898 ms; evaluator clocks and lap splits remain cartridge fields. |
...
| Current OBS57 Puffer full-race MP4 | **PASS.** Selected randomized-wave policy from time zero through the frozen official finish; 92.23 s, 5,534 decoded frames, full decode clean. |
