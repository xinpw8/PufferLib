...
The defensible conclusion is narrow. A real statically recompiled game core existed underneath a misleading demo and an incorrect initial environment. The current repairs establish a credible fixed-scenario CPU environment in the native PufferLib 5.0 trainer. Final throughput and learning quality are still pending.
...
| The environment was already suitable for high-throughput learning. | The original transcript reported about 20.9 K steps/s and called it roughly 14,000 times below the unrelated CUDA proxy. The current adapter, observations, action contract, reward, and Puffer backend differ. | **Unproven.** A clean current-5.0 benchmark is pending. |
| A short training smoke test proved the agent could race. | Earlier reward and observation bugs allowed distance gaming, hid the required buoy side, used a short-discount horizon, and sometimes tested shortened episodes. | **False.** Production three-lap convergence and evaluation remain pending. |
...
| Hidden buoy passing side | Observation exposes the signed pass point using the course lateral vector and the next pass point. | Pass-point geometry tests and a zero-miss production three-lap controller using only public observations. | Learned-policy performance pending. |
...
| Discounted reward clawback bug | Route progress and checkpoint terms use `gamma * Phi(next) - Phi(current)`, with terminal potential zero. | Discounted failure regression. | Learning benefit pending. |
| Learner silently clipped rewards to `[-1, 1]` | Wave Race opts out of the generic learner clamp, preserving potential telescoping and the configured terminal magnitudes. | Compile-time adapter gate plus native train/eval acceptance. | Hyperparameter quality pending. |
| Discount made long failures cheaper than short failures | Every nonterminal transition now charges `reward_fail * (1-gamma)`, making the discounted task return of a zero-miss failure exactly `-reward_fail` at any duration. | Held-A failure, 2,997-decision no-op failure, and three-lap discounted-return regressions. | Learning benefit pending. |
| Short-horizon `gamma=0.995` with frameskip 1 | Production uses frameskip 4, horizon 64, GAE lambda `0.98`, and gamma `0.9995`; trainer gamma is injected into the environment. | Frameskip and gamma-coupling tests plus the two-seed temporal pilot. | Full learning acceptance pending. |
...
| OpenMP initialization could pin rollout creators | Caller affinity is captured and restored before rollout pthread creation. | Affinity equality tests. | Final bound/unbound performance matrix pending. |
| Old Puffer 4 integration | Adapter ported to the native upstream PufferLib 5.0 trainer and CPU environment ABI. | Native build and deterministic adapter harness. | Final clean build/test record should be retained with the benchmark. |
| Post-train evaluation inherited partial training episodes | The trainer now waits for rollout workers, resets every environment, clears transition state, uploads reset observations, zeros recurrent state, reinitializes action RNG state, and synchronizes before evaluation. | Central reset path in `src/pufferl.cu`; retained acceptance still uses a fresh process and explicit checkpoint. | Final learning and evaluation results pending. |
...
| --- | --- |
| Current PufferLib 5.0 performance | **PENDING.** Retain wall time, decisions/s, guest updates/s, CPU utilization, GPU utilization, memory, agents, buffers, threads, horizon, and frameskip. |
| Production learning | **PENDING.** Complete the configured five-million-transition run, evaluate the checkpoint, and compare with retained no-op/random baselines. |
| Seed sensitivity | **PENDING.** Use at least three training seeds with a common evaluation protocol. |
