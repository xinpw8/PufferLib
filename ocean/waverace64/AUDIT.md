...
The defensible conclusion is narrower. A real statically recompiled game core existed underneath a misleading demo and an incorrect initial environment. The repairs establish a fixed-course CPU simulator in the native PufferLib 5.0 trainer, a 57-value state observation with local wave samples, native speed, and power, plus a human-readable state evaluator. Production now selects per episode from a finite validated pool of complete cartridge-booted race-start states. A controlled renderer-free simulator benchmark measured only a 1.44% median throughput reduction for 128 variants. A clean complete fixed-wave OBS57 run measured 51,949.565260 decisions/s, and its selected checkpoint completed 128/128 deterministic and 509/512 held-out stochastic official three-lap evaluations. Those learning results predate wave randomization and do not validate the new distribution. The simulator remains host-CPU-bound, randomized-wave training and broader training-seed statistics are pending, and broad game parity remains unproven. Earlier OBS43 and OBS55 checkpoints are incompatible.
...
| The environment is currently proven performant. | The complete fixed-wave trainer run reached 51,949.565260 decisions/s. Under an identical renderer-free simulator protocol, K=128 randomized and fixed medians were 56,314.635 and 57,135.018 decisions/s. | **Supported for steady simulator rollout; complete randomized training is pending.** The measured median difference is 1.44%. Simulation remains CPU-bound. |
| A short training smoke test proved the agent could race. | Earlier reward and observation bugs allowed distance gaming, hid the required buoy side, used a short-discount horizon, and sometimes tested shortened episodes. Later fixed-wave evidence used a complete OBS57 run and fresh official three-lap evaluation. | **False for the original smoke; fixed-wave learning is supported.** Seed 901 finished 128/128 deterministic and 509/512 held-out stochastic episodes. Randomized-wave training is pending. |
...
| Every reset reused the same wave field | Production selects a coherent race-start state from a finite pool of complete cartridge boots. Checked 4 KiB deltas reconstruct donor RDRAM while each environment keeps its own native stack and context. Fixed mode remains available for parity regressions. | All 128 seed-902 donors matched reconstructed owners across reset state and two 32-decision scripts; complete fields were unique, exact replay passed, and the held-out seed-2902 pool had zero overlap. | Complete randomized-wave training and policy quality are pending. |
...
| Discount made long failures cheaper than short failures | Every nonterminal transition now charges `reward_fail * (1-gamma)`, making the discounted task return of a zero-miss failure exactly `-reward_fail` at any duration. | Held-A failure, no-op failure, discounted-return regressions, and fixed-wave OBS57 learning. | Randomized-wave learning remains pending. |
| Short-horizon `gamma=0.995` with frameskip 1 | The checked-in configuration uses frameskip 2, horizon 128, 10,485,760 decisions, GAE lambda `0.98`, and gamma `0.9997499687421851`; trainer gamma is injected into the environment. | Frameskip and gamma-coupling tests plus a complete fixed-wave run. This preserves prior guest-update exposure, per-second discount, and a 12.8 s recurrent horizon. | Complete randomized training and continuous process-specific resource telemetry remain pending. |
...
| Old Puffer 4 integration | Adapter ported to the native upstream PufferLib 5.0 trainer and CPU environment ABI. | Complete fixed-wave OBS57 CUDA training, fresh CUDA evaluation, and deterministic adapter harness. | Randomized-wave training remains pending. |
...
## Randomized reset acceptance, historical fixed-wave learning, and remaining items

The checked-in randomized reset contract has direct simulator throughput, exhaustive pool-reconstruction, held-out-pool separation, and reset-stress evidence. It does not yet have a complete trainer run, a compatible trained checkpoint, a fresh policy evaluation, or a full-race capture. All learning, policy-evaluation, checkpoint, and MP4 evidence below predates authentic per-episode wave-pool selection and remains fixed-wave provenance only.
...
| --- | --- |
| Randomized-wave OBS57 PufferLib 5.0 performance | **PASS FOR STEADY ADAPTER ROLLOUT; COMPLETE TRAINER PENDING.** Identical-protocol K=128 median was 56,314.635 decisions/s versus 57,135.018 fixed, a 1.44% reduction. |
| Randomized-wave OBS57 learning | **PENDING.** Seed 901 was trained on the fixed-wave reset contract. |
...
| Compatible randomized-wave checkpoint | **PENDING.** The retained 438,272-byte seed-901 checkpoint is fixed-wave provenance only. |
| Human state evaluation | **PASS STATIC REGRESSIONS.** Valid shared Puffer model, compact truthful Time Trials HUD, native clocks/speed/power, two-way human toggle, terminal freeze, and visible wave-relative motion are verified. |
| Current OBS57 Puffer full-race MP4 | **PASS.** Cursor-free native CUDA policy from time zero through the frozen official finish; 89.133 s and 5,348 decoded frames. |
