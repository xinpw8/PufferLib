...
This writes `frame-000000.png` through `frame-000149.png`. At 60 rendered frames/s and ten policy actions per simulated second, `--capture-every=6` retains one image per policy action and spans 15 simulated seconds. The example above retains every twelfth rendered frame, or every second policy action, and spans 30 simulated seconds. `--capture-every=N` keeps every Nth rendered frame. `--capture-hidden` hides the window but still requires a working display and OpenGL context. `--capture-fast` removes normal render pacing and must not be used as a training-throughput measurement. Capture cannot be combined with `--headless`; `--headless` selects metrics-only CPU evaluation and does not call the renderer.
...
| Fixed-state pixels and model lifecycle | Two captures were byte-identical; pixel hash `cadfc805209a87f0`; the shared Puffer GLB loaded as a valid model and unloaded cleanly |
| Buoy label orientation | Both incoming and outgoing face bases put glyph-positive X on the viewer's right; live-race captures show a normal `R` on red and `L` on yellow |
...
The pixel hash is a regression baseline for the tested Raylib/OpenGL software stack. It is not asserted to be portable across graphics drivers, and it is not a reference N64 framebuffer hash. The post-training letter-orientation repair only changed `waverace64_render.h`; the rendered and headless simulator trajectory hash remained `bb75cd7a071f6551`. The retained regression log has SHA-256 `258bb244031c0d450e977214af70e90a0a583c7712d90625873d3fc709304b42`. Corrected red and yellow live-race captures have SHA-256 `70e06085bbbf9e206f79cf7b8753c6d318cda197a600b91d147363139bdfecb8` and `c38fa8391ce9ff4867d0181a0dfc43915e2eeac1f18536575186e6f3f9a79c81`.
...
| --- | --- |
| Full renderer-free training throughput | **MEASURED:** clean seed 900 reached 51,949.565260 Puffer SPS; 103,899.130521 internal native updates/s is a secondary diagnostic |
