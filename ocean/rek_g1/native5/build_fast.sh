...
{ printf 'backend=semantic_cuda\ncontrol_hz=50\ncpu_physics=0\npython_runtime=0\n';
  sha256sum "$task_source/fast_runtime.cu" "$task_source/fast_assets.cpp" \
    ...
    "$task_source/round_reward.h" "$task_source/normalized_reward.h" "$task_source/policy_feature_mask.h" "$task_source/owned_yaw_observation.h" \
    "$task_source/fast_observable_balance.h" "$task_source/observable_balance.h" \
