...
if [[ ${REK_FAST_REWARD:-point_difference_v1} == round_outcome_v1 ]];then
    if [[ -v REK_FAST_REWARD_GAMMA && "$REK_FAST_REWARD_GAMMA" != "${REK_TRAIN_GAMMA:-0.999}" ]];then
        printf 'Round reward gamma must match learner gamma\n' >&2;exit 2
    fi
    # One source for the learner's discount and its score potential.
    task_environment+=("REK_FAST_REWARD_GAMMA=${REK_TRAIN_GAMMA:-0.999}")
fi
# Only these task-specific values are recorded. Never dump the shell environment.
for task_key in REK_OBSERVATION_SCHEMA REK_POLICY_FEATURE_MASK REK_POLICY_ACTION_STRIDE REK_FAST_YAW_COMMAND REK_FAST_CONTACT_ENTRY REK_FAST_CONTACT_VELOCITY REK_FAST_REWARD REK_FAST_SCORING REK_FAST_GEOMETRY REK_FAST_CONTACT_SUBSTEPS REK_FAST_OPPONENT REK_FAST_OBSERVATION REK_FAST_OPPONENT_MODE REK_FAST_RANDOM_RESETS REK_FAST_RESET_GAP_MIN REK_FAST_RESET_GAP_MAX REK_FAST_RESET_HEADING_SPREAD_RAD REK_FAST_SHAPING_WEIGHT REK_FAST_SHAPING_GAMMA REK_FAST_SHAPING_TARGET REK_FAST_SHAPING_BEARING_WEIGHT REK_FAST_CONTACT_POTENTIAL REK_FROZEN_OPPONENT_FRACTION;do
    if [[ -v "$task_key" ]];then task_environment+=("$task_key=${!task_key}");fi
done
if [[ -n ${REK_POLICY_FEATURE_MASK:-} ]];then
    test -f "$REK_POLICY_FEATURE_MASK"
    sha256sum "$REK_POLICY_FEATURE_MASK" > "$task_output/policy-feature-mask.sha256"
    cp "$REK_POLICY_FEATURE_MASK" "$task_output/policy-feature-mask.bin"
fi
if [[ -n ${REK_FAST_CONTACT_POTENTIAL:-} ]];then
    test -f "$REK_FAST_CONTACT_POTENTIAL"
    [[ ${REK_FAST_SHAPING_GAMMA:-} == "${REK_TRAIN_GAMMA:-0.999}" ]] || { printf 'Contact shaping gamma must match learner gamma\n' >&2;exit 2; }
    sha256sum "$REK_FAST_CONTACT_POTENTIAL" > "$task_output/contact-potential.sha256"
    cp "$REK_FAST_CONTACT_POTENTIAL" "$task_output/contact-potential.json"
fi
task_command=(env "${task_environment[@]}" "$task_build/puffer-rek-native5" train --headless
    ...
    --sweep.metric="${REK_TRAIN_METRIC:-perf/train}" --sweep.downsample="${REK_TRAIN_DOWNSAMPLE:-64}" --base.run_id="$task_run"
    --base.seed="${REK_TRAIN_BASE_SEED:-73}"
