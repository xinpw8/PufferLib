#!/usr/bin/env bash
# Explicitly parity-relaxed, native CUDA training against a frozen native5 policy.
set -euo pipefail
...
task_command=(env REK_PHYSICS_BACKEND=puffysics REK_PUFFYSICS_STABILIZATION=joint_cold_start REK_ALLOW_CPU_EVALUATION=0
    ...
    --base.run_id=native5-frozen20s --base.checkpoint_interval=8
    ...
    --env.opponent_checkpoint="$task_opponent" --env.opponent_sha256="$task_opponent_hash"
    --env.opponent_observation_encoding=scaled_polar_xy
    --env.opponent_hidden_size=256 --env.opponent_num_layers=2
    --env.opponent_precision=0 --env.opponent_legacy_fast_hidden=0
    --env.opponent_deterministic=0)
...
printf '\n' >> "$task_output/command.txt"
{
    ...
    sha256sum "$task_executable" "$task_opponent" "$task_assets/model.two_fighter_arena.xml" \
