...
__global__ void exercise_idle(Parameters* params,FastFrame* frames,Arena* states,IdleResults* out){
    ...
    for(unsigned tick=0;tick<256;tick++){
        ...
        for(int side=0;side<2;side++){
            ...
            __syncwarp();
            geom_pair_contacts_warp(v,optimized,side,oh,op,lane,scratch);
            if(lane==0){
...
int main(){try{
    ...
    exercise_idle<<<1,32>>>(params,frames,states,out);rek5::cuda_check(cudaDeviceSynchronize());
    std::printf("{\"test\":\"contact_warp_production_equivalence\",\"checks\":%u,\"failures\":%u,\"first_failure\":%u,\"steps\":%u,\"idle_calls\":%u,\"active_calls\":%u,\"points\":%u,\"resets\":%u,\"invalid_apex_calls\":%u,\"passed\":%s}\n",
