...
constexpr int ticks=259, substeps=10;
...
int move(int lane) { return lane<2?3:10; }
...
void identity() {
    std::printf("{\"event\":\"schedule_identity\",\"classification\":\"candidate_from_reset_behavior_check\",\"authentic_replay\":false,\"authentic_parity\":false,\"arenas\":4,\"yaw_conditions\":[-1,0,1],\"decision_hz\":50,\"physical_step_s\":0.002,\"substeps\":10,\"warmup_ticks\":50,\"yaw_lead_ticks\":25,\"first_attack_tick\":76,\"ticks_per_reset\":259,\"moves\":[3,10],\"categories\":[23,26],\"move_duration_ticks\":[45,134],\"opponent_action\":1,\"actor_lanes\":[\"move3_attack\",\"move3_no_attack_control\",\"move10_attack\",\"move10_no_attack_control\"],\"post_attack_category\":0,\"post_duration_category\":1,\"unknowns\":[\"authentic_full_qvel\",\"authentic_SONIC_history\",\"authentic_clip_phase\",\"server_action_acceptance_and_timing\",\"opponent_action_history\"]}\n");
}
void self_test() {
    int checks=0;
    auto check=[&](bool ok){if(!ok)throw std::runtime_error("Schedule self-test failed");++checks;};
    check(category(3)==23);check(category(10)==26);check(registry[19-16]==9);
