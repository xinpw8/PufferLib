#!/usr/bin/env bash
set -euo pipefail
[[ $# == 2 ]] || { printf 'Usage: bash run_physical_fall_exposure_probe.sh BUILD_DIRECTORY NEW_RUN_DIRECTORY\n' >&2; exit 2; }
...
export REK_NATIVE5_REWARD=normalized_points_falls_v1
...
task_run=(timeout 180s "$task_build/physical-fall-exposure-probe" "$task_assets/model.two_fighter_arena.xml" "$task_export" "$task_assets" "$task_features" "$task_controller/model_encoder.batch8.onnx" "$task_controller/model_decoder.batch8.onnx")
{ printf '%s\n' "REK_PHYSICS_BACKEND=$REK_PHYSICS_BACKEND" "REK_ALLOW_CPU_EVALUATION=$REK_ALLOW_CPU_EVALUATION" "REK_NATIVE5_REWARD=$REK_NATIVE5_REWARD";printf '%q ' "${task_run[@]}";printf '\n'; } > "$task_out/run-command.txt"
sha256sum "$task_build/physical-fall-exposure-probe" "$task_build/physical_fall_exposure_probe.cpp" "$task_assets/model.two_fighter_arena.xml" "$task_export" "$task_assets"/*.json "$task_features"/*.json "$task_controller"/*.onnx "$REK_MUJOCO_KERNEL_CATALOG" "$REK_MUJOCO_CONDITIONAL_PTX" > "$task_out/provenance.txt"
