...
set -euo pipefail
[[ $# == 2 ]] || { printf 'Usage: bash run_physical_left_front_probe.sh BUILT_DIRECTORY NEW_RUN_DIRECTORY\n' >&2; exit 2; }
...
task_run=(timeout 120s "$task_build/physical-left-front-probe" --run "$task_assets/model.two_fighter_arena.xml" "$task_export" "$task_assets" "$task_features" "$task_controller/model_encoder.batch8.onnx" "$task_controller/model_decoder.batch8.onnx")
...
{ date -u --iso-8601=ns;sha256sum "$task_build/physical-left-front-probe" "$task_build/physical_left_front_probe.cpp" "$task_assets/model.two_fighter_arena.xml" "$task_export" "$task_assets"/*.json "$task_features"/*.json "$task_controller"/*.onnx "$REK_MUJOCO_KERNEL_CATALOG" "$REK_MUJOCO_CONDITIONAL_PTX"; } > "$task_out/provenance.txt"
