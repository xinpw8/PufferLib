...
set -euo pipefail
[[ $# == 1 ]] || { printf 'Usage: bash build_physical_left_front_probe.sh NEW_OUTPUT_DIRECTORY\n' >&2; exit 2; }
task_out=$1
mkdir "$task_out"
...
cp "$task_source/physical_left_front_probe.cpp" "$task_source/build_physical_left_front_probe.sh" "$task_source/run_physical_left_front_probe.sh" "$task_out/"
...
g++ -std=c++17 -O2 -DREK_SCHEDULE_CPU_ONLY "$task_out/physical_left_front_probe.cpp" -o "$task_out/schedule-cpu"
...
{ date -u --iso-8601=ns; sha256sum "$task_out/physical-left-front-probe" "$task_out/physical_left_front_probe.cpp" "$task_out/build_physical_left_front_probe.sh" "$task_out/run_physical_left_front_probe.sh" "$task_native/runtime_api.h" "$task_native/../g1_semantic_action_table.c" "$task_native/../native_motion_routes.c" "${task_objects[@]}"; } > "$task_out/build-provenance.txt"
