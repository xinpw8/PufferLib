...
export REK_NATIVE5_REWARD=normalized_points_falls_v1 REK_OBSERVATION_SCHEMA=rek.native5.observable_balance.v1
export REK_PHYSICAL_OPPONENT=recovered_bot1_g1_v1
...
task_command=(timeout --signal=TERM --kill-after=10s 300s "$task_build/physical-bot1-runtime-probe"
    "$task_assets/model.two_fighter_arena.xml" "$task_export" "$task_assets" "$task_features"
...
{ printf '%s\n' "REK_PHYSICS_BACKEND=$REK_PHYSICS_BACKEND" "REK_ALLOW_CPU_EVALUATION=$REK_ALLOW_CPU_EVALUATION" \
    "REK_NATIVE5_REWARD=$REK_NATIVE5_REWARD" "REK_OBSERVATION_SCHEMA=$REK_OBSERVATION_SCHEMA" \
    "REK_PHYSICAL_OPPONENT=$REK_PHYSICAL_OPPONENT" "REK_MUJOCO_KERNEL_CATALOG=$REK_MUJOCO_KERNEL_CATALOG" \
    "REK_MUJOCO_CONDITIONAL_PTX=$REK_MUJOCO_CONDITIONAL_PTX" "REK_MUJOCO_CONDITIONAL_SHA256=$REK_MUJOCO_CONDITIONAL_SHA256"
    printf '%q ' "${task_command[@]}"; printf '\n'; } > "$task_output/command.txt"
hostname > "$task_output/host.txt"
sha256sum "$task_build/physical-bot1-runtime-probe" "$task_assets/model.two_fighter_arena.xml" "$task_export" \
    "$task_controller/model_encoder.batch8.onnx" "$task_controller/model_decoder.batch8.onnx" \
    "$REK_MUJOCO_KERNEL_CATALOG" "$REK_MUJOCO_CONDITIONAL_PTX" > "$task_output/provenance.txt"
set +e
/usr/bin/time -v -o "$task_output/process-timing.txt" "${task_command[@]}" > "$task_output/stdout.jsonl" 2> "$task_output/stderr.txt"
task_status=$?
set -e
printf '%s\n' "$task_status" > "$task_output/exit-code.txt"
cat "$task_output/stdout.jsonl"
