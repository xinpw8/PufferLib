...
task_command=("$task_nvcc" -std=c++17 -O3 -arch=sm_121
    ...
    "$task_source/test_training_autoreset.cu" "$task_build/fast_assets.o" "$task_build/cJSON.o"
    -L"$task_mujoco" -Xlinker=-rpath -Xlinker="$task_mujoco" -l:libmujoco.so.3.7.0 -lcrypto
    -o "$task_out/test-autoreset")
printf '%q ' "${task_command[@]}" > "$task_out/build.command.txt"
{ date -u --iso-8601=seconds; hostname; id; "$task_nvcc" --version; nvidia-smi -L;
  sha256sum "$task_source/test_training_autoreset.cu" "$task_source/fast_runtime.cu" "$task_build/fast_assets.o" "$task_build/cJSON.o";
} > "$task_out/provenance.txt"
"${task_command[@]}" > "$task_out/build.stdout.txt" 2> "$task_out/build.stderr.txt"
printf '%q\n' "$task_out/test-autoreset" > "$task_out/test.command.txt"
set +e
"$task_out/test-autoreset" > "$task_out/test.stdout.jsonl" 2> "$task_out/test.stderr.txt"
task_status=$?
set -e
printf '%s\n' "$task_status" > "$task_out/exit-code.txt"
sha256sum "$task_out/test-autoreset" >> "$task_out/provenance.txt"
