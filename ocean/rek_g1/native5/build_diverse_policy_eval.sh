...
task_command=("$task_cuda/bin/nvcc" -std=c++17 -O3 -arch=sm_121 -I"$task_source"
  "$task_source/diverse_policy_eval.cu" "$task_build/fast_runtime.o" "$task_build/fast_assets.o"
  ...
  -o "$task_output/diverse-policy-eval")
...
sha256sum "$task_source/diverse_policy_eval.cu" "$task_build/fast_runtime.o" "$task_build/fast_assets.o" \
  "$task_build/native_policy.o" "$task_build/cJSON.o" "$task_output/diverse-policy-eval" > "$task_output/build-hashes.txt"
readelf -d "$task_output/diverse-policy-eval" > "$task_output/elf-dependencies.txt"
if rg -qi '(libpython|libtorch|libonnxruntime)' "$task_output/elf-dependencies.txt";then printf 'Unexpected inference dependency\n' >&2;exit 2;fi
printf 'Built native batched evaluator: %s/diverse-policy-eval\n' "$task_output"
