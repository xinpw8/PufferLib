...
using rek5_native_contact::TargetCount;
...
struct Model{
 ...
 std::array<Geom,12> strikes[2];std::array<Geom,TargetCount> targets[2];int bodies[2][30]{};int bone_for_body[256]{};
 explicit Model(const char* path,const char* expected){
  ...
  for(int side=0;side<2;side++){
   ...
   const std::string prefix=side?"opponent__":"player__";
   for(int i=0;i<TargetCount;i++){int g=mj_name2id(m.get(),mjOBJ_GEOM,(prefix+rek5_native_contact::TargetNames[i]).c_str());require(g>=0,"target missing");targets[side][i]=geom(g);require(targets[side][i].local.kind==rek5_native_contact::TargetKinds[i],"target kind mismatch");require(m->geom_margin[g]==0&&m->geom_gap[g]==0,"nonzero target margin or gap");}
  }
 }
...
struct Query{Shape strikers[12],targets[TargetCount],old_strikers[6],old_targets[3];float hit[3];int kick;};
// Keep the original three-target masks for a paired regression. Full-target
// counts avoid truncating 108 pair bits into a 64-bit mask or JSON number.
struct Result{unsigned long long pairs,compatible_pairs;int all_pairs,all_compatible,old_pairs,old_compatible;int target_compatible[TargetCount];float point_striker,point_target;};
...
__global__ void contacts(const Query* queries,Result* results,int n){
 ...
 for(int i=0;i<12;i++){
  ...
  for(int j=0;j<TargetCount;j++)if(rek5_primitive::overlap(q.strikers[i],q.targets[j])){
   out.all_pairs++;if(compatible){out.all_compatible++;out.target_compatible[j]++;}
   if(j<3){unsigned long long bit=1ull<<(3*i+j);out.pairs|=bit;if(compatible)out.compatible_pairs|=bit;}
  }
 }
 for(int j=0;j<TargetCount;j++)out.point_target=fminf(out.point_target,point_distance(q.targets[j],q.hit));
 ...
 try{
  ...
  for(size_t event=0;event<events.size();event++){
   ...
   for(int step=-30;step<=30;step++){
    ...
    for(int i=0;i<TargetCount;i++){q.targets[i]=world(model.targets[side^1][i],*p[side^1]);if(i<3)q.old_targets[i]=sphere_union(q.targets,i,i+1);}
  ...
  for(size_t i=0;i<queries.size();i++){
   ...
   std::fprintf(output.get(),"{\"event\":\"pose_contact_query\",\"round\":%d,\"hit_sequence\":%d,\"score_sequence\":%d,\"scorer\":%d,\"points\":%d,\"is_kick\":%d,\"receipt_lag_seconds\":%.9g,\"pose_sequences\":[%d,%d],\"pose_unity_frames\":[%d,%d],\"pose_age_seconds\":[%.9g,%.9g],\"primitive_pair_mask\":%llu,\"part_compatible_pair_mask\":%llu,\"all_target_pair_count\":%d,\"all_target_part_compatible_count\":%d,\"target_part_compatible_counts\":[",e.round,e.hit.seq,e.score.seq,e.score.scorer,e.score.points,e.hit.kick,m.lag,m.seq[0],m.seq[1],m.frames[0],m.frames[1],m.ages[0],m.ages[1],r.pairs,r.compatible_pairs,r.all_pairs,r.all_compatible);
   for(int j=0;j<TargetCount;j++)std::fprintf(output.get(),"%s%d",j?",":"",r.target_compatible[j]);
   std::fprintf(output.get(),"],\"legacy_pair_count\":%d,\"legacy_part_compatible_count\":%d,\"hit_point_striker_distance_m\":%.9g,\"hit_point_target_distance_m\":%.9g}\n",r.old_pairs,r.old_compatible,r.point_striker,r.point_target);
  }
  require(std::fflush(output.get())==0,"output flush failed");
  std::printf("{\"event\":\"pose_contact_replay\",\"model_sha256\":\"%s\",\"capture_sha256\":[\"%s\",\"%s\"],\"rounds\":2,\"paired_strikes\":%zu,\"queries\":%zu,\"unpaired_hits\":[%d,%d],\"unpaired_scores\":[%d,%d],\"pose_counts\":[[%zu,%zu],[%zu,%zu]],\"target_primitives\":%d,\"core_target_primitives\":3,\"lag_scan_seconds\":[-0.25,0.25],\"lag_step_seconds\":0.008333333333333333,\"max_pose_age_seconds\":0.075,\"contact_execution\":\"CUDA_static_overlap\",\"cpu_physics\":false,\"python_runtime\":false,\"authoritative_contact_time\":false,\"negative_outcome_labels\":0,\"canned_motion_used\":false,\"round2_fit\":false,\"authentic_parity\":false}\n",argv[2],captures[0].digest.c_str(),captures[1].digest.c_str(),events.size(),queries.size(),unpaired_hits[0],unpaired_hits[1],unpaired_scores[0],unpaired_scores[1],captures[0].poses[0].size(),captures[0].poses[1].size(),captures[1].poses[0].size(),captures[1].poses[1].size(),TargetCount);return 0;
