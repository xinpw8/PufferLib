...
task_compile=(/usr/local/cuda/bin/nvcc -std=c++17 -O3 -arch=sm_121 -I"$task_source"
    "$task_source/fast_input_probe.cu" "$task_build/fast_runtime.o" "$task_build/fast_assets.o" "$task_build/cJSON.o"
    -L"$task_mujoco" -Xlinker=-rpath -Xlinker="$task_mujoco" -l:libmujoco.so.3.7.0 -lcrypto -o "$task_output/fast-input-probe")
task_run=("$task_output/fast-input-probe" "$task_assets/model.two_fighter_arena.xml"
...
{ hostname;id;date -u --iso-8601=seconds;sha256sum "$task_source/fast_input_probe.cu" "$task_source/fast_input_probe.sh" "$task_build/fast_runtime.o" "$task_build/fast_assets.o" "$task_output/fast-input-probe"; } > "$task_output/provenance.txt"
set +e
/usr/bin/time -v -o "$task_output/process-timing.txt" timeout --signal=TERM --kill-after=10s 120s "${task_run[@]}" > "$task_output/stdout.txt" 2> "$task_output/stderr.txt"
task_status=$?
set -e
