...
const out='/home/spark-advantage/rek-training/timing500-ppo-refresh-live-20260924-r1';
...
function configuration(template,checkpoint,checkpointSha,index){
 ...
 const cfg=structuredClone(template),seed=1501+index,label='refresh-s'+seed;
...
function main(){
 ...
 const receipt={created_utc:new Date().toISOString(),target_wins:18,rounds:20,stop_nonwins:3,checkpoint_sha256:checkpointSha,parent_checkpoint_sha256:parentSha,driver_sha256:driverSha,controller_sha256:controllerSha,observation_schema:template.observation_schema,feature_mask_sha256:template.feature_mask_sha256,bridge_sha256:template.relay.at(-1),source_stage:source,
  hypothesis:'One-epoch native PPO from actual C2 behavior on all five closed C2 rounds, retaining wins and losses; LR3e-5, actual score-delta/5 and QPC five-second half-life. Runtime, encoder, masks and unforced sampled actions are unchanged. New RNG seeds do not control server randomness.',artifacts,plan,controller_started:false};
