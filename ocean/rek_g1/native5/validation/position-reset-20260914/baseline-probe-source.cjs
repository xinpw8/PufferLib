...
(async()=>{
  ...
  if(mode==='paired_i'){
    ...
    await attack(18,await settle());
    for(let t=0;t<100;t++)await step(0,'no_input_between_attacks');
    trial='paired_i_second_attack';
    await attack(18,await request('snapshot'));
  }
  const result={status:'completed',runtime:'preserved_semantic_cuda_v2',round_seconds:300,arenas:1,
  ...
  fs.writeFileSync(path.join(output,'summary.json'),JSON.stringify({summary:result,trials:summaries},null,2)+'\n',{flag:'wx'});
