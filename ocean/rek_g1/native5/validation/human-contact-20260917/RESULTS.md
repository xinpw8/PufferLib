...
The [recorded-pose replay](POSE-CONTACT.md) has now run on both rounds, with no fitting. All 33 strike/score pairs were processed. It exposed six missing hip targets; the [source-derived nine-target correction](ALL-SCORING-TARGETS.md) increased compatible overlap from 3/33 to 7/33 at receipt, and from 19/33 to 31/33 within plus/minus 250 ms. These receipt-time diagnostics do not establish server collision timing or definitive hit regions. The training and frozen-checkpoint results above predate the target correction. The next task is to explain the remaining contact-reconstruction gap while preserving ambiguous action attribution.
