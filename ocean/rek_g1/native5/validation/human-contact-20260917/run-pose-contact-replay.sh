...
# Usage: bash run-pose-contact-replay.sh PRIVATE_STAGE NEW_RUN_LABEL [SOURCE_TREE]
set -euo pipefail
umask 077
[[ $# == 2 || $# == 3 ]] || exit 2
task_stage=$(realpath "$1")
task_label=$2
task_source_tree=${3:-source}
[[ $task_stage == /home/spark-advantage/rek-training/human-pose-replay-20260917-r1 ]] || exit 2
[[ $task_label =~ ^run-[a-z0-9-]+$ ]] || exit 2
[[ $task_source_tree =~ ^source(-[a-z0-9-]+)?$ ]] || exit 2
...
set -x
task_source=$task_stage/$task_source_tree/ocean/rek_g1/native5
task_vendor=$task_stage/$task_source_tree/vendor
...
sha256sum "$task_source/pose_contact_replay.cu" "$task_source/primitive_contacts.cuh" "$task_source/native_contact_geometry.h" "$task_vendor/cJSON.c" "$task_vendor/cJSON.h" "$task_source/validation/human-contact-20260917/run-pose-contact-replay.sh" "$task_model" "$task_round1" "$task_round2" "$task_mujoco/libmujoco.so.3.7.0" > "$task_run/input-hashes.txt"
