...
test('only checkpoint, RNG seed and output change; runtime, masks and controls remain exact',()=>{
 ...
 assert.deepEqual(t,before);assert.equal(r.seed,1401);assert.equal(r.label,'lr3e5-s1401');assert.equal(r.cfg.worker[1],'/new/policy.bin');
 for(const k of ['encoder','relay','live_attack_gate','feature_mask_sha256'])assert.deepEqual(r.cfg[k],t[k]);
 assert.deepEqual(r.cfg.worker.slice(4),t.worker.slice(4));assert.equal(configuration(t,'/new','b'.repeat(64),19).seed,1420);
