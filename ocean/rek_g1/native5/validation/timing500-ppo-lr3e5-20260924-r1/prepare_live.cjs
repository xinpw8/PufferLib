...
const out='/home/spark-advantage/rek-training/timing500-ppo-lr3e5-live-20260924-r1';
...
function configuration(template,checkpoint,checkpointSha,index){
 ...
 const cfg=structuredClone(template),seed=1401+index,label='lr3e5-s'+seed;
...
function main(){
 ...
 const receipt={created_utc:new Date().toISOString(),target_wins:18,rounds:20,stop_nonwins:3,checkpoint_sha256:checkpointSha,parent_checkpoint_sha256:oldSha,driver_sha256:driverSha,controller_sha256:controllerSha,observation_schema:template.observation_schema,feature_mask_sha256:template.feature_mask_sha256,bridge_sha256:template.relay.at(-1),source_stage:source,
  hypothesis:'One-epoch native PPO sibling from the same actual5b19 eight-round behavior dataset/replay; only learning rate changes1e-5 to3e-5. Same runtime, encoder, masks and unforced sampled actions. New RNG seeds do not control server randomness.',artifacts,plan,controller_started:false};
...
if(require.main===module)main();module.exports={configuration,oldSha};
