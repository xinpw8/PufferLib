...
int main(int argc,char** argv) {
    try {
        require(argc==9,"Usage: replay-balance8 DATA MIGRATED_CHECKPOINT SHA256 NEW_REPLAY DERIVED_IDENTITY EXACT_RECORDED_WORKER ORIGINAL_DATA ORIGINAL_REPLAY");
        ...
        require(data.format_version==4,"derived balance8 format required");
        const auto original_data=rek_authentic::load(argv[7]);
        const auto original_bytes=rek_authentic::read_file(argv[8]);
        const auto original_replay=rek_authentic::decode_replay(original_bytes,original_data);
        require(original_data.digest==data.original_dataset_sha256&&
            rek_authentic::sha256(original_bytes.data(),original_bytes.size())==data.original_replay_sha256&&
            original_data.checkpoint_sha256==data.checkpoint_sha256&&original_data.rows.size()==data.rows.size(),"original behavior binding mismatch");
        for(size_t i=0;i<data.rows.size();i++){
            require(data.rows[i].action==original_data.rows[i].action&&data.rows[i].behavior_seed==original_data.rows[i].behavior_seed,"derived behavior row identity mismatch");
            for(int c=0;c<223;c++)if(!rek_balance8::column(c))require(std::memcmp(&data.rows[i].obs[c],&original_data.rows[i].obs[c],4)==0,"protected observation changed");
        }
        if(identity) {
            ...
            require(data.derived_teacher_sha256==argv[3],"derived teacher checkpoint identity mismatch");
        }
        ...
        std::memcpy(output.data(),"REKBR004",8); put<uint32_t>(output,8,4); put<uint32_t>(output,12,data.rows.size());
        put<uint32_t>(output,16,rek_authentic::REPLAY_ROW_BYTES); put_hash(output,24,data.digest);
        put_hash(output,56,data.checkpoint_sha256); put<uint64_t>(output,88,identity?0:73);
        ...
        for(size_t i=0;i<data.rows.size();++i) {
            ...
            const auto& old=original_replay.rows[i];
            require(std::memcmp(logits.data(),old.logits.data(),34*sizeof(float))==0&&
                std::memcmp(&host_stats[0],&old.old_logprob,sizeof(float))==0&&
                std::memcmp(&host_stats[1],&old.old_value,sizeof(float))==0,
                "migrated teacher differs from recorded behavior logits/value/logprob; no replay published");
            std::memcpy(output.data()+offset,original_bytes.data()+offset,rek_authentic::REPLAY_ROW_BYTES);
        ...
        char report[3000]; const int count=std::snprintf(report,sizeof(report),
            ...
            "rek.authentic_derived_balance8_replay.v4",
            owned_yaw?"\"observation_schema\":\"rek.native5.scaled_polar_xy.owned_yaw_v2\",\"trajectory_format\":\"REKRL002\",\"replay_format\":\"REKBR002\",":"",
            data.rows.size(),rounds,matching,data.digest.c_str(),data.checkpoint_sha256.c_str(),digest.c_str(),minimum_logprob,maximum_logprob);
        ...
        if(identity) {
            ...
            replace("\"training_performed\":false","\"training_performed\":false,\"derived_teacher_was_recorded_worker\":false,\"original_logprobs_values_logits_copied_bitwise\":true,\"derived_teacher_sha256\":\""+checkpoint+"\"");
