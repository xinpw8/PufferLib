...
inline Dataset decode(const std::vector<unsigned char>& b) {
    ...
    const bool identity=derived||std::memcmp(b.data(),"REKRL003",8)==0;
    ...
    require(scalar<uint32_t>(&b[8])==(derived?4:identity?3:1) && scalar<uint32_t>(&b[12])==OBS &&
    ...
    d.format_version=derived?4:identity?3:1;
    if(identity) {
...
inline Replay decode_replay(const std::vector<unsigned char>& b,const Dataset& d) {
    const bool derived=d.format_version==4,identity=d.format_version>=3;
    require(b.size()>=REPLAY_HEADER_BYTES && std::memcmp(b.data(),derived?"REKBR004":identity?"REKBR003":"REKBR001",8)==0 &&
        scalar<uint32_t>(&b[8])==(derived?4:identity?3:1) && scalar<uint32_t>(&b[12])==d.rows.size() &&
