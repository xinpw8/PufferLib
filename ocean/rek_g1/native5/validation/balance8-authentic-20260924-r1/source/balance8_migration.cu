...
bool changed_index(size_t index,const Layout& l){
    return index>=l.encoder_begin&&index<l.encoder_begin+size_t(H)*O&&pa::column(int((index-l.encoder_begin)%O));
}
std::vector<float> transform(const std::vector<float>& input,const Layout& l){
    ...
    auto output=input;
    for(int h=0;h<H;h++)for(int column=0;column<O;column++)if(pa::column(column))
...
Json parse_sidecar(const Bytes& bytes,const std::string& digest){
    ...
    const auto* artifacts=cJSON_GetObjectItemCaseSensitive(json.get(),"artifacts");
    const auto* checkpoint=cJSON_GetObjectItemCaseSensitive(artifacts,"checkpoint");
    const auto* hash=cJSON_GetObjectItemCaseSensitive(checkpoint,"sha256");
    require(cJSON_IsString(schema)&&schema->valuestring&&std::string(schema->valuestring)==pa::kBaseSchema,
        "source checkpoint is not bound to legacy scaled_polar_xy.v1");
    require(cJSON_IsString(hash)&&hash->valuestring&&std::string(hash->valuestring)==digest,"source sidecar SHA mismatch");
    for(auto shape:std::array<std::pair<const char*,int>,2>{{{"hidden_size",H},{"num_layers",L}}}){
...
void migrate(const char* path,const char* expected,const char* identity_path,const char* output_path){
    ...
    const std::string old_sidecar=std::filesystem::canonical(identity_path).string(),new_sidecar=output.string()+".policy-schema.json";
    ...
    replace_string(receipt.get(),"checkpoint",output.string());replace_string(receipt.get(),"migration","zero_eight_measured_balance_encoder_columns");
    replace_string(receipt.get(),"source_observation_schema",pa::kBaseSchema);
    ...
    replace_string(receipt.get(),"identity_semantics","derived migrated teacher; never the recorded worker checkpoint; immutable original behavior replay remains denominator");
    ...
    for(int i=0;i<O;i++)if(pa::column(i))cJSON_AddItemToArray(columns,cJSON_CreateNumber(i));
...
void cpu_test(){
    ...
    size_t changed=0;for(size_t i=0;i<l.count;i++)changed+=changed_index(i,l);require(changed==2048,"wrong column count");
    ...
    auto broken=fresh;broken[pa::kColumns[0]]=-0.f;bool rejected=false;
    ...
    for(int i=0;i<O;i++)base[i]=pa::column(i)?0.f:float((i%17)-8)/8.f;
    unsigned dot_products=0;
    for(int fixture=0;fixture<34;fixture++){
        augmented=base;for(int c:pa::kColumns)augmented[c]=float(fixture-17)/4.f;
        for(int h=0;h<H;h++){
    ...
    std::cout<<"{\"cpu_migration_tests\":\"passed\",\"registered_parameters\":"<<l.count
        <<",\"encoder_begin\":0,\"zeroed_entries\":"<<changed<<",\"protected_floats\":456960,\"encoder_dot_products\":"
...
int main(int argc,char** argv){try{
    ...
    require(argc==6&&std::string(argv[1])=="--migrate","usage: balance8-migration --cpu-self-test | --migrate INPUT EXPECTED_SHA RECORDED_IDENTITY_JSON NEW_CHECKPOINT");
    migrate(argv[2],argv[3],argv[4],argv[5]);return 0;
}catch(const std::exception& e){std::cerr<<"balance8 migration: "<<e.what()<<'\n';return 1;}}
