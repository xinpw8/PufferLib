...
struct GaeTargets {
    ...
    GaeTargets(const rek_authentic::Dataset& data, const rek_authentic::Replay& replay, bool complete_mc_zero_baseline) {
        ...
        std::printf("{\"phase\":\"cuda_target_reference_check\",\"target_mode\":\"%s\",\"rows\":%zu,\"max_abs_error\":%.12g,\"training_target_producer\":\"cuda\",\"advantage_mean\":%.12g,\"advantage_std\":%.12g,\"advantage_min\":%.12g,\"advantage_max\":%.12g}\n",
            complete_mc_zero_baseline ? "complete_mc_zero_baseline" : "frozen_value_gae", input.size(), maximum,
...
int main(int argc, char** argv) {
    try {
        require(argc >= 13 && argc <= 16, "Usage: authentic-ppo DATA REPLAY INITIAL SHA256 NEW_OUTPUT EPOCHS LR HORIZON CLIP VF_CLIP VF_COEF ENT_COEF [--allow-bounded-bf16-batch|--allow-distributional-bf16-batch] [--targets=complete-mc-zero-baseline] [--observation-schema=rek.native5.scaled_polar_xy.owned_yaw_v2]");
        ...
        require(!(owned_yaw&&balance8)&&balance8==(data.format_version==4),"explicit derived balance8 schema required");
        ...
        require((balance8?data.derived_teacher_sha256:replay.checkpoint_sha256) == argv[4], "initial checkpoint identity mismatch");
        ...
        require(!complete_mc_zero_baseline || vf_coef == 0, "complete MC zero baseline requires vf_coef=0");
        struct stat st{};
        ...
        if (balance8) std::printf("{\"observation_schema\":\"%s\",\"trajectory_format\":\"REKRL004\",\"replay_format\":\"REKBR004\",\"derived_initial_teacher_sha256\":\"%s\",\"original_dataset_sha256\":\"%s\",\"original_replay_sha256\":\"%s\",\"derived_teacher_was_recorded_worker\":false}\n",rek_balance8::kSchema,data.derived_teacher_sha256.c_str(),data.original_dataset_sha256.c_str(),data.original_replay_sha256.c_str());
