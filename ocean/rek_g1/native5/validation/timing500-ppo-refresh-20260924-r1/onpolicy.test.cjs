...
test('native command keeps one epoch, LR3e-5, H128, complete MC, existing PPO options',()=>{
 const p={schema:'rek.scorecredit5s_onpolicy_run_plan.v1',stage:'/private',rounds:5,minimum_completed_rounds:5,dataset:{path:'data'},identity:{path:'identity'},behavior:{path:'actual',sha256:PINS.checkpoint},worker:{path:'worker'},binaries:{replay:{path:'replayer'},trainer:{path:'trainer'}},observation_schema:SCHEMA,settings:{epochs:1,learning_rate:.00003,horizon:128,clip:.2,vf_clip:.2,vf_coef:0,entropy:.001,targets:'complete_mc_zero_baseline',reward:'received-score-delta-div5-half-life-5s.v1'}};
 const argv=command(p,'train');assert.deepEqual(argv.slice(6,13),['1','.00003','128','.2','.2','0','.001']);assert(argv.includes('--targets=complete-mc-zero-baseline'));assert.equal(command(p,'replay')[3],PINS.checkpoint);assert.equal(command(p,'diagnose')[6],'0');
});

test('exact five-round native capture selection rejects omissions, duplicates and wrong source scope',()=>{
 const api=require('./selection.cjs'),bytes=fs.readFileSync(path.join(__dirname,'native-captures-final.json'));
 assert.equal(sha(bytes),api.NATIVE_CAPTURE_SHA);const capture=JSON.parse(bytes),ids=capture.rounds.map(r=>r.label);
 assert.deepEqual(ids,['timingppo-s1301-retry7','timingppo-s1302-retry2','timingppo-s1303-retry3','timingppo-s1304','timingppo-s1305']);
 assert.equal(api.validateNativeSelection(capture,ids),capture);
 for(const change of [x=>x.rounds.pop(),x=>x.rounds[1].label=x.rounds[0].label,x=>x.rounds[1].native_source=x.rounds[0].native_source,
  x=>x.rounds[0].strict_validation_passed=false,x=>x.rounds[0].native_source.file='/other/native.jsonl',x=>x.campaign_end_utc='2026-09-24T11:01:00Z']){
   const c=structuredClone(capture);change(c);assert.throws(()=>api.validateNativeSelection(c,ids));
 }
});
