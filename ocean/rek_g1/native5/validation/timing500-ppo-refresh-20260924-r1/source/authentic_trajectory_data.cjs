...
async function readTrial(directory, sequence, gamma20ms, lambda20ms, observationSchema = LEGACY_SCHEMA, behavior = null) {
  ...
  const coveragePath=path.join(directory,'controlled-policy-coverage.json');
  const coverageReceipt=fs.existsSync(coveragePath)?json(coveragePath):null;
  const s = summary.value, roundId = s.round_identity_sha256, slot = s.local_slot;
  requireValue((contact.value.completed_policy_round === true || coverageReceipt !== null) && contact.value.terminal_evidence_consistent === true &&
  ...
  await read('relay.stdout.jsonl', x => {
    if (x.event === 'g1_policy_state') {
      if(coverageReceipt)coverageSources.push(coverageApi.compactSource(x));
  ...
  let controlledCoverage=null;
  if(coverageReceipt){
    const measured=coverageApi.deriveCoverage(s,contact.value,coverageSources,[...acks.values()],ordered[0].seq);
    const bindings=[{...summary.input,file:'trial/summary.json'},
      {...contact.input,file:'contact-analysis/summary.json'},
      ...['relay.stdout.jsonl','worker.stdin.jsonl'].map(name=>({...inputs.find(x=>x.file===name),file:'trial/'+name}))]
      .map(({file,bytes,sha256})=>({file,bytes,sha256}));
    controlledCoverage=coverageApi.validateReceipt(coverageReceipt.value,measured,bindings);
  }
  ...
  return { rows, ledger, report: { trial_id: path.basename(directory), sequence, round_identity_sha256: roundId,
    ...
    ...(controlledCoverage?{legacy_completed_policy_round:contact.value.completed_policy_round,controlled_policy_coverage:controlledCoverage}:{}),
    ...
    inputs: [...inputs, { ...summary.input, file: 'trial/summary.json' },
      ...(coverageReceipt?[{...coverageReceipt.input,file:'controlled-policy-coverage.json'}]:[]),
      { ...contact.input, file: 'contact-analysis/summary.json' },
