...
function command(plan,mode){
 ...
 assert(['train','diagnose'].includes(mode));assert.deepEqual(plan.settings,{epochs:1,learning_rate:.00003,horizon:128,clip:.2,vf_clip:.2,vf_coef:0,entropy:.001,targets:'complete_mc_zero_baseline',reward:'received-score-delta-div5-half-life-5s.v1'});
 return[plan.binaries.trainer.path,plan.dataset.path,replay,plan.behavior.path,plan.behavior.sha256,path.join(plan.stage,mode+'-score-delta/policy.bin'),mode==='train'?'1':'0','.00003','128','.2','.2','0','.001','--allow-distributional-bf16-batch','--targets=complete-mc-zero-baseline','--observation-schema='+plan.observation_schema];
}
async function main(){
