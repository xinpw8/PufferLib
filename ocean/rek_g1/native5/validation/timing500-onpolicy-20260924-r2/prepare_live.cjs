...
const out='/home/spark-advantage/rek-training/timing500-ppo-live-20260924-r1';
...
function configuration(template,checkpoint,checkpointSha,index){
 ...
 const cfg=structuredClone(template),seed=1301+index,label='timingppo-s'+seed;
...
function main(){
 ...
 const receipt={created_utc:new Date().toISOString(),target_wins:18,rounds:20,stop_nonwins:3,checkpoint_sha256:checkpointSha,parent_checkpoint_sha256:oldSha,driver_sha256:driverSha,controller_sha256:controllerSha,observation_schema:template.observation_schema,feature_mask_sha256:template.feature_mask_sha256,bridge_sha256:template.relay.at(-1),source_stage:source,
  hypothesis:'Native PPO on the complete actual parent cohort, preserving measured live observations and recurrent cadence; scoring-only rewards. Same runtime, encoder, masks and unforced sampled actions. New RNG seeds do not control server randomness.',artifacts,plan,controller_started:false};
