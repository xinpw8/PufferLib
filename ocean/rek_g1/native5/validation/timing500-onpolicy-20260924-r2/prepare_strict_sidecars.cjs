...
async function main() {
  ...
  write(path.join(evidence, 'manifest.json'), {schema: 'rek.scorecredit5s_onpolicy_strict_evidence.v1', created_utc: new Date().toISOString(),
    selection:provenance(process.argv[2]), minimum_completed_rounds:selection.minimum_completed_rounds, checkpoint_sha256: checkpoint, modules, rounds, original_files_changed: false,
