...
function derive(stage){
 ...
 assert.equal(source.subarray(320,352).toString('hex'),'9a875c347b512bb46dde25481be75d276ea1b4c0799886b30aaf03be6ae1c5ce');
 const rows=source.readUInt32LE(20);assert.equal(source.length,HEADER+rows*ROW);assert.equal(rows,15078);
 ...
 for(const round of identity.rounds){
  ...
  assert(Math.abs(sum-margin)<1e-6);assert.equal(zeroPolicy,1);
 ...
 const nextIdentity={...identity,schema:'rek.authentic_behavior_identity.v5',reward_contract:CONTRACT,recorded_behavior_checkpoint:true,weight_migration:false,
  original_identity:sourceIdentity,original_dataset:sourceDataset,
...
if(require.main===module){assert.equal(process.argv.length,3);console.log(JSON.stringify(derive(process.argv[2])));}
