...
constexpr size_t DERIVED_HEADER_BYTES = 480;
...
struct Dataset {
    ...
    std::string derived_teacher_sha256, original_dataset_sha256, original_replay_sha256;
};
...
inline Dataset decode(const std::vector<unsigned char>& b) {
    ...
    if(derived){
        d.derived_teacher_sha256=hex(&b[384],32);d.original_dataset_sha256=hex(&b[416],32);
        d.original_replay_sha256=hex(&b[448],32);
        require(d.derived_teacher_sha256!=std::string(64,'0')&&d.original_dataset_sha256!=std::string(64,'0')&&
            d.original_replay_sha256!=std::string(64,'0'),"missing derived identity bindings");
    }
