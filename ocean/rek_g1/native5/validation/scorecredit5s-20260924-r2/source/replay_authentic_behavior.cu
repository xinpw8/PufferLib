...
int main(int argc,char** argv) {
    try {
        require(argc==5||argc==6||argc==7||argc==8,"Usage: replay-authentic-behavior DATA CHECKPOINT SHA256 NEW_REPLAY_BINARY [IDENTITY_JSON EXACT_WORKER | --observation-schema=rek.native5.scaled_polar_xy.owned_yaw_v2]");
        const bool owned_yaw=argc==6;
        const bool balance8=argc==8;
        require(!balance8 || std::string(argv[7])=="--observation-schema=rek.native5.scaled_polar_xy.balance8_v1","unknown replay balance8 option");
        ...
        const bool identity=data.format_version==3||data.format_version==5;
        const bool score_delta=data.format_version==5;
        require(identity==(argc==7||balance8),"v3 requires exact identity JSON and worker executable");
        if(identity) {
        ...
        std::memcpy(output.data(),score_delta?"REKBR005":identity?"REKBR003":owned_yaw?"REKBR002":"REKBR001",8); put<uint32_t>(output,8,score_delta?5:identity?3:owned_yaw?2:1); put<uint32_t>(output,12,data.rows.size());
        ...
        char report[3000]; const int count=std::snprintf(report,sizeof(report),
            ...
            score_delta?"rek.authentic_behavior_replay.v5":identity?"rek.authentic_behavior_replay.v3":owned_yaw?"rek.authentic_behavior_replay.owned_yaw_v2":"rek.authentic_behavior_replay.v1",
        ...
        if(balance8) report_text.insert(1,"\"observation_schema\":\"rek.native5.scaled_polar_xy.balance8_v1\",\"teacher_was_recorded_worker\":true,");
        if(identity) {
