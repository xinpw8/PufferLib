...
// Isolated diagnostic: both rows use external actions, with only opponent rows
// overwritten after policy inference. The exact policy-produced rows survive.
__global__ void set_overrides(uint8_t* values,int arenas,int side){int row=blockIdx.x*blockDim.x+threadIdx.x;if(row<arenas*2)values[row]=1;}
__global__ void neutral_opponent(float* actions,int arenas,int side){int row=blockIdx.x*blockDim.x+threadIdx.x;if(row<arenas*2&&(row%2)!=side)actions[row]=1;}
__global__ void validate_neutral(const float* actions,const uint8_t* overrides,int* failures,int arenas,int side){int row=blockIdx.x*blockDim.x+threadIdx.x;if(row<arenas*2&&(row%2)!=side&&(actions[row]!=1||overrides[row]!=1))atomicOr(failures,16);}
...
int main(int argc,char** argv){
    try{
        require(argc==10||argc==12,"Usage: afk-policy-eval RUNTIME_JSON CHECKPOINT SHA256 ARENAS ROUNDS_PER_SIDE SEED sampled|greedy bf16|fp32 NEW_RECORDS_JSONL [HIDDEN LAYERS]");
        ...
        for(int side=0;side<2;side++){
            ...
            long long wins=0,losses=0,ties=0,points=0,opponent_points=0,falls=0,opponent_falls=0,zero_points=0;
            for(const auto& r:host){require(r.completed==static_cast<unsigned long long>(r.episode+1)&&r.duration_ticks>0,"Incomplete terminal record");if(r.winner<0)ties++;else if(r.winner==side)wins++;else losses++;
                points+=r.points[side];if(r.points[side]==0)zero_points++;opponent_points+=r.points[side^1];falls+=r.falls[side];opponent_falls+=r.falls[side^1];
            ...
            std::printf("{\"event\":\"stationary_target_diagnostic\",\"policy_side\":%d,\"games\":%d,\"round_seconds\":%.9g,\"zero_points_games\":%lld,\"opponent\":\"neutral_action_1_override_1\",\"neutral_rows_checked_each_tick\":true,\"policy_rows_preserved\":true}\n",side,arenas*rounds,cfg.round_seconds,zero_points);
            cuda_ok(cudaGraphExecDestroy(executable));cuda_ok(cudaGraphDestroy(graph));
        }
        std::printf("{\"event\":\"frozen_policy_evaluation\",\"backend\":\"semantic_cuda\",\"checkpoint_sha256\":\"%s\",\"precision\":\"%s\",\"observation_encoding\":\"scaled_polar_xy\",\"selection\":\"%s\",\"policy_rng_seed\":%d,\"wins\":%lld,\"losses\":%lld,\"draws\":%lld,\"win_rate\":%.9g,\"execution_wall_seconds\":%.9g,\"environment_randomizes_seed\":false,\"greedy_repeats_duplicate_fixed_fixtures\":%s,\"opponent\":\"neutral_action_1_override_1\",\"both_sides\":true,\"terminal_recurrent_reset\":true,\"failure_bits\":0,\"python_runtime\":false,\"cpu_physics\":false,\"training_sps\":null,\"diagnostic\":%s,\"round_feature_override\":%s}\n",digest.c_str(),precision.c_str(),selection.c_str(),seed,total_wins,total_losses,total_ties,double(total_wins)/double(total_wins+total_losses+total_ties),total_wall,selection=="greedy"?"true":"false",diagnostic?"true":"false",diagnostic_json.c_str());
