...
task_ptx=("$task_cuda/bin/nvcc" -std=c++17 -arch=sm_121 -ptx "$task_source/conditional.cu" -o "$task_output/conditional.ptx")
...
task_compile=(g++ -std=c++17 -O2 -ffp-contract=off -I"$task_cuda/include" -I"$task_mujoco/include" -I"$task_root/vendor"
    ...
    "$task_source/kernel_program.cpp" "$task_source/conditional.cpp" "$task_source/constraint_schedule.cpp"
    "$task_source/solver_schedule.cpp" "$task_source/solver_smoke.cpp" "$task_output/cJSON.o"
    -L"$task_cuda/lib64/stubs" -L"$task_mujoco" -Wl,-rpath,"$task_mujoco"
    -lcuda -lcrypto -l:libmujoco.so.3.7.0 -o "$task_output/solver-smoke")
{ printf '%q ' "${task_ptx[@]}";printf '\n';printf '%q ' "${task_cjson[@]}";printf '\n';printf '%q ' "${task_compile[@]}";printf '\n'; } > "$task_output/build-command.txt"
"${task_ptx[@]}" > "$task_output/build.stdout.txt" 2> "$task_output/build.stderr.txt"
...
task_command=("$task_output/solver-smoke" "$task_model" "$task_catalog" "$task_output/conditional.ptx" "$task_hash")
...
{ date -u --iso-8601=seconds;hostname;id;uname -m;nvidia-smi -L;
  sha256sum "$task_model" "$task_catalog" "$task_output/solver-smoke" "$task_output/conditional.ptx";
  find "$task_source" -maxdepth 1 -type f -exec sha256sum {} +; } > "$task_output/provenance.txt"
readelf -d "$task_output/solver-smoke" > "$task_output/elf-dependencies.txt"
