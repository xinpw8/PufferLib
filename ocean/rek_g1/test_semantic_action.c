...
int main(void) {
    ...
    require(rek_g1_semantic_start(&scheduler, command, 4) ==
        REK_G1_SEMANTIC_OK, "kick_segment_starts");
    for (int tick = 0; tick < 4; tick++) {
        RekG1SemanticTick result = rek_g1_semantic_tick(
            &scheduler, timing, 1, tick > 0);
        require(result.status == REK_G1_SEMANTIC_OK, "kick_tick_accepted");
        require(result.kick_start_edge == (tick == 0),
            "kick_edge_dispatched_once");
        require(result.kick_active, "kick_active_for_full_duration");
        ...
        require(result.input.yaw == 0 &&
            ...
            "yaw_preempted_for_full_kick_duration");
        require(result.segment_complete == (tick == 3),
            "kick_completes_exactly");
    }
    ...
    require(rek_g1_semantic_start(&scheduler, command, 4) ==
        REK_G1_SEMANTIC_OK, "post_translation_kick_proposed");
    ...
    require(blocked.input.released_edges == REK_G1_HELD_FORWARD,
        "translation_release_visible_on_kick_tick");
    require(blocked.kick_blocked && blocked.segment_complete,
        "blocked_kick_segment_terminates");
    require(rek_g1_semantic_tick(&scheduler, timing, 1, 0).status ==
        REK_G1_SEMANTIC_NO_ACTIVE_SEGMENT,
        "blocked_kick_is_not_queued_or_retried");

    require(rek_g1_semantic_start(&scheduler, command, 4) ==
        REK_G1_SEMANTIC_OK, "busy_gate_kick_proposed");
    ...
    require(rek_g1_semantic_start(&scheduler, command, 4) ==
        ...
        "locomotion_category_cannot_encode_kick");
