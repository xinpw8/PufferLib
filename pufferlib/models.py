...
# BET added Impala/Procgen below
class Impala(Policy):
...
class ProcgenBET(Policy):
    def __init__(self, env, *args, input_size=64, hidden_size=64, output_size=64,  flat_size, channels_last=True):
        super().__init__(env)
        self.channels_last = channels_last
        if self.channels_last:
        ...
        conv_seqs += [
            ...
            nn.ReLU(),
            nn.Linear(flat_size, hidden_size),
            # nn.Linear(in_features=shape[0] * shape[1] * shape[2], out_features=output_size),
            nn.ReLU(),
        ]
    ...
    def encode_observations(self, observations):
        hidden = self.network(observations.permute((0, 3, 1, 2)) / 255.0)  # "bhwc" -> "bchw"
        return hidden, None
