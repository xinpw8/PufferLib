...
#define CRASH_NOOP_DURATION_CAR_VS_CAR \
    90  // How long controls are disabled after car v car collision
#define CRASH_NOOP_DURATION_CAR_VS_ROAD \
    20  // How long controls are disabled after car v road edge collision
...
#define LOGICAL_VANISHING_Y \
    (VANISHING_POINT_Y +    \
     12)  // Separate logical vanishing point for cars disappearing
#define PLAYER_MAX_Y \
    (ACTION_HEIGHT - CAR_HEIGHT)  // Max y is car length from bottom
#define PLAYER_MIN_Y \
    (ACTION_HEIGHT - CAR_HEIGHT - 9)  // Min y is ~2 car lengths from bottom
...
#define CURVE_PLAYER_SHIFT_FACTOR \
    0.025f  // Moves player car towards outside edge of curves
...
typedef struct GameState {
    ...
    int flag_timer;
    unsigned char
        show_left_flag;         // true shows left flag, false shows right flag
    int victory_display_timer;  // Timer for how long victory effects have been
                                // displayed
    // Variables for scrolling digits
    float score_digit_offsets[SCORE_DIGITS];  // Offset for scrolling effect for
                                              // each digit
    int score_digit_currents[SCORE_DIGITS];   // Current digit being displayed
                                              // for each position
    int score_digit_nexts[SCORE_DIGITS];  // Next digit to scroll in for each
                                          // position
    unsigned char
        score_digit_scrolling[SCORE_DIGITS];  // Scrolling state for each digit
...
typedef struct Enduro {
    ...
    float collision_cooldown_car_vs_road;  // Timer for car vs road edge
                                           // collisions
    int drift_direction;  // Which way player car drifts whilst nooped after
                          // crash vs car
...
static int get_furthest_lane(
    const Enduro* env) {  // Get furthest lane from player
    if (env->lane == 0) {
        return 2;
    } else if (env->lane == 2) {
        return 0;
        ...
        float road_center_x =
            (env->last_road_left + env->last_road_right) / 2.0f;
...
Log aggregate_and_clear(LogBuffer* logs) {
    ...
    for (int i = 0; i < logs->idx; i++) {
        ...
        log.step_rew_car_passed_no_crash +=
            logs->logs[i].step_rew_car_passed_no_crash /= logs->idx;
        ...
        log.collisions_player_vs_car +=
            logs->logs[i].collisions_player_vs_car /= logs->idx;
        log.collisions_player_vs_road +=
            logs->logs[i].collisions_player_vs_road /= logs->idx;
    }
    logs->idx = 0;
    return log;
}
...
void init(Enduro* env, int seed, int env_index) {
    ...
    if (seed == 0) {  // Activate with seed==0
        ...
        float total_day_duration =
            BACKGROUND_TRANSITION_TIMES[NUM_BACKGROUND_TRANSITIONS - 1];
        env->elapsed_time_env =
            ((float)xorshift32(&env->rng_state) / (float)UINT32_MAX) *
            total_day_duration;
        env->current_day_time_index = 0;
    ...
    env->score = 0;
    env->speed = MIN_SPEED;
    memcpy(env->day_transition_times, BACKGROUND_TRANSITION_TIMES,
           sizeof(BACKGROUND_TRANSITION_TIMES));
    env->step_count = 0;
    ...
    for (int i = 0; i < 4; i++) {
        ...
        env->gear_acceleration_thresholds[i] =
            gearSpeedIncrement / (gearTime * TARGET_FPS);
    ...
    if (env->rng_state == 0) {
        ...
        env->elapsed_time_env =
            ((float)rand() / (float)RAND_MAX) * total_day_duration;
        env->current_day_time_index = 0;
        env->day_time_index = 0;
        env->previous_day_time_index = 0;
        ...
        env->previous_day_time_index = (env->current_day_time_index > 0)
                                           ? env->current_day_time_index - 1
                                           : NUM_BACKGROUND_TRANSITIONS - 1;
    }
    env->terminals[0] = 0;
    env->truncateds[0] = 0;
    env->rewards[0] = 0.0f;
...
float quadratic_bezier(float bottom_x, float control_x, float top_x, float t) {
    ...
    return one_minus_t * one_minus_t * bottom_x +
           2.0f * one_minus_t * t * control_x + t * t * top_x;
}

float road_edge_x(const Enduro* env, float y, float offset,
                  unsigned char left) {
    ...
    if (fabsf(env->current_curve_factor) < 0.01f) {
        ...
        // Slight curve
        float curve_offset = (env->current_curve_factor > 0 ? -30.0f : 30.0f) *
                             fabsf(env->current_curve_factor);
    ...
    if (env->wiggle_active && y >= env->wiggle_y &&
        y <= env->wiggle_y + env->wiggle_length) {
...
void add_enemy_car(Enduro* env) {
    ...
    if (player_lane == 0)
        ...
        float road_center_x = (road_edge_x(env, env->player_y, 0, true) +
                               road_edge_x(env, env->player_y, 0, false)) /
                              2.0f;
    ...
    float dynamic_spacing_factor =
        ((float)rand() / (float)RAND_MAX) * 6.0f + 0.5f;
...
void update_road_curve(Enduro* env) {
    ...
    float speed_scale =
        0.5f + ((fabsf(env->speed) / MAX_SPEED) * (MAX_SPEED - MIN_SPEED));
    float vanishing_point_transition_speed =
        VANISHING_POINT_TRANSITION_SPEED + speed_scale;
    ...
    env->current_step_threshold =
        (float)step_thresholds[*current_curve_stage % 3];
    (*steps_in_current_stage)++;
    if (*steps_in_current_stage >= step_thresholds[*current_curve_stage % 3]) {
        env->target_curve_factor =
            (float)curve_directions[*current_curve_stage % 3];
        *steps_in_current_stage = 0;
        *current_curve_stage = (*current_curve_stage + 1) % 3;
    }
    size_t step_thresholds_size =
        sizeof(step_thresholds) / sizeof(step_thresholds[0]);
    size_t curve_directions_size =
        sizeof(curve_directions) / sizeof(curve_directions[0]);
    ...
    for (size_t i = 0; i < max_size; i++) {
        ...
        adjusted_curve_directions[i] =
            curve_directions[i % curve_directions_size];
    }
    env->current_step_threshold =
        (float)adjusted_step_thresholds[*current_curve_stage % max_size];
    (*steps_in_current_stage)++;
    if (*steps_in_current_stage >=
        adjusted_step_thresholds[*current_curve_stage]) {
        env->target_curve_factor =
            (float)adjusted_curve_directions[*current_curve_stage % max_size];
        *steps_in_current_stage = 0;
    ...
    if (env->current_curve_factor < env->target_curve_factor) {
        env->current_curve_factor =
            fminf(env->current_curve_factor + CURVE_TRANSITION_SPEED,
                  env->target_curve_factor);
    } else if (env->current_curve_factor > env->target_curve_factor) {
        env->current_curve_factor =
            fmaxf(env->current_curve_factor - CURVE_TRANSITION_SPEED,
                  env->target_curve_factor);
    }
    if (fabsf(env->current_curve_factor) < 0.1f) {
        env->current_curve_direction = CURVE_STRAIGHT;
    } else {
        env->current_curve_direction =
            (env->current_curve_factor > 0) ? CURVE_RIGHT : CURVE_LEFT;
    }
    env->base_target_vanishing_point_x =
        VANISHING_POINT_X_LEFT -
        env->t_p * (VANISHING_POINT_X_LEFT - VANISHING_POINT_X_RIGHT);
    float target_shift =
        (float)env->current_curve_direction * CURVE_VANISHING_POINT_SHIFT;
    env->target_vanishing_point_x =
        env->base_target_vanishing_point_x + target_shift;
    if (env->current_vanishing_point_x < env->target_vanishing_point_x) {
        env->current_vanishing_point_x = fminf(
            env->current_vanishing_point_x + vanishing_point_transition_speed,
            env->target_vanishing_point_x);
    } else if (env->current_vanishing_point_x > env->target_vanishing_point_x) {
        env->current_vanishing_point_x = fmaxf(
            env->current_vanishing_point_x - vanishing_point_transition_speed,
            env->target_vanishing_point_x);
    }
...
void compute_observations(Enduro* env) {
    ...
    float player_y_norm =
        (PLAYER_MAX_Y - env->player_y) / (PLAYER_MAX_Y - PLAYER_MIN_Y);
    ...
    for (int i = 0; i < MAX_ENEMIES; i++) {
        ...
        if (car->y > VANISHING_POINT_Y && car->y < PLAYABLE_AREA_BOTTOM) {
            ...
            float car_y_norm =
                (PLAYABLE_AREA_BOTTOM - (car->y - buffer_y)) / (MAX_DISTANCE);
    ...
    float drift_magnitude = env->current_curve_factor *
                            CURVE_PLAYER_SHIFT_FACTOR * (float)fabs(env->speed);
    float drift_direction = (env->current_curve_factor > 0) ? 1.0f : -1.0f;
    float max_drift_magnitude = CURVE_PLAYER_SHIFT_FACTOR * MAX_SPEED;
    float normalized_drift_magnitude =
        (float)fabs(drift_magnitude) / max_drift_magnitude;
    ...
    obs[obs_index++] =
        fmodf(env->elapsed_time_env, total_day_length) / total_day_length;
    // Cars to pass
    ...
    for (int l = 0; l < NUM_LANES; l++) {
        float normalized_distance =
            is_lane_empty[l] ? 1.0f : nearest_car_distance[l] / MAX_DISTANCE;
...
float calculate_enemy_spawn_interval(const Enduro* env) {
    ...
    if (day_index == 0) {
        ...
        float reduction = (float)(day_index - NUM_MAX_SPAWN_INTERVALS + 1) *
                          DAILY_INTERVAL_REDUCTION;
        max_spawn_interval = clamp_spawn_interval(
            base_interval - reduction, MIN_POSSIBLE_INTERVAL, base_interval);
    }
    max_spawn_interval = fmaxf(max_spawn_interval, MIN_SPAWN_INTERVAL);
    float speed_range = MAX_SPEED - MIN_SPEED;
    float speed_factor =
        speed_range > 0.0f ? (env->speed - MIN_SPEED) / speed_range : 0.0f;
    ...
    return MIN_SPAWN_INTERVAL +
           (1.0f - speed_factor) * interval_range * SPAWN_SCALING_FACTOR;
}
...
void c_step(Enduro* env) {
    ...
    if (env->collision_cooldown_car_vs_car <= 0 &&
        ...
        movement_amount =
            (((env->speed - MIN_SPEED) / (MAX_SPEED - MIN_SPEED)) + 0.3f);
        if (is_snow_stage) {
        ...
        if (env->drift_direction == 0) {
            ...
            for (int i = 0; i < env->num_enemies; i++) {
                const Car* car = &env->enemy_cars[i];
                if ((car->lane == 1 ||
                     car->lane == env->lane + env->drift_direction) &&
                    (car->y > env->player_y)) {
    ...
    // Road curve shift
    float curve_shift = -env->current_curve_factor * CURVE_PLAYER_SHIFT_FACTOR *
    ...
    float curve_vanishing_point_shift =
        (float)env->current_curve_direction * CURVE_VANISHING_POINT_SHIFT;
    env->vanishing_point_x =
        env->base_vanishing_point_x + curve_vanishing_point_shift;
    // Wiggle logic
    if (env->wiggle_active) {
        float min_wiggle_period = 5.8f;
        float max_wiggle_period = 0.3f;
        float speed_normalized =
            (env->speed - MIN_SPEED) / (MAX_SPEED - MIN_SPEED);
        speed_normalized = fmaxf(0.0f, fminf(1.0f, speed_normalized));
        float current_wiggle_period =
            min_wiggle_period - powf(speed_normalized, 0.25f) *
                                    (min_wiggle_period - max_wiggle_period);
        env->wiggle_speed =
            (MAX_DISTANCE) / (current_wiggle_period * TARGET_FPS);
    ...
    if (env->player_x <= road_left || env->player_x >= road_right) {
        ...
        env->player_x =
            fmaxf(road_left + 1, fminf(road_right - 1, env->player_x));
    }
    // Enemy logic
    for (int i = 0; i < env->num_enemies; i++) {
        Car* car = &env->enemy_cars[i];
        ...
        if (env->speed > 0 && car->last_y < env->player_y + CAR_HEIGHT &&
            ...
            car->passed = true;
        } else if (env->speed < 0 && car->last_y > env->player_y &&
                   car->y <= env->player_y) {
    ...
    if (env->enemy_spawn_timer >= env->enemy_spawn_interval) {
        ...
        if (env->num_enemies < MAX_ENEMIES) {
            float clump_probability =
                fminf((env->speed - MIN_SPEED) / (MAX_SPEED - MIN_SPEED), 1.0f);
            int num_to_spawn = 1;
            if (((float)rand() / (float)RAND_MAX) < clump_probability) {
                num_to_spawn = 1 + rand() % 2;  // up to 3 cars
            }
            int occupied_lanes[NUM_LANES] = {0};
            for (int i = 0; i < num_to_spawn && env->num_enemies < MAX_ENEMIES;
                 i++) {
                int lane;
                do {
                    lane = rand() % NUM_LANES;
    ...
    if (env->current_day_time_index == 0 &&
        env->previous_day_time_index == 15) {
...
void load_textures(GameState* gameState, Enduro* env) {
    ...
    gameState->spritesheet =
        LoadTexture("resources/enduro/enduro_spritesheet.png");
    for (int i = 0; i < 16; ++i) {
    ...
    for (int i = 0; i < SCORE_DIGITS; i++) {
        if (gameState->score_digit_scrolling[i]) {
            ...
            if (gameState->score_digit_offsets[i] >= DIGIT_HEIGHT) {
                gameState->score_digit_offsets[i] = 0.0f;
                gameState->score_digit_currents[i] =
                    gameState->score_digit_nexts[i];
                gameState->score_digit_scrolling[i] = false;
            }
        }
    }
...
void render_scoreboard(GameState* gameState, Enduro* env) {
    ...
    for (int i = 0; i < SCORE_DIGITS; ++i) {  // Draw scrolling score digits
        ...
        if (i == SCORE_DIGITS - 1) {
            current_asset_index =
                gameState->yellow_digit_indices[current_digit_index];
            next_asset_index =
                gameState->yellow_digit_indices[next_digit_index];
        } else {
        ...
        if (gameState->score_digit_scrolling[i]) {
            ...
            DrawTexturePro(gameState->spritesheet, src_rect_current,
                           dest_rect_current, (Vector2){0, 0}, 0.0f, WHITE);
            Rectangle src_rect_next = src_rect_next_full;
            src_rect_next.y += (digit_height - (int)offset);
            src_rect_next.height = (int)offset;
            Rectangle dest_rect_next = {digit_x, (float)score_start_y,
                                        (float)digit_width,
                                        (float)((int)offset)};
            DrawTexturePro(gameState->spritesheet, src_rect_next,
                           dest_rect_next, (Vector2){0, 0}, 0.0f, WHITE);
        } else {
    ...
    if (env->day_completed) {
        ...
        Rectangle dest_rect = {(float)cars_x, (float)cars_y,
                               flag_src_rect.width, flag_src_rect.height};
        DrawTexturePro(gameState->spritesheet, flag_src_rect, dest_rect,
        ...
        DrawTextureRec(gameState->spritesheet, car_src_rect, car_position,
                       WHITE);
...
void render_mountains(GameState* gameState, Enduro* env) {
    int mountain_index =
        gameState->mountain_indices[env->current_day_time_index];
    ...
    BeginScissorMode(PLAYABLE_AREA_LEFT, 0, SCREEN_WIDTH - PLAYABLE_AREA_LEFT,
                     SCREEN_HEIGHT);
    for (int x = (int)mountain_x; x < SCREEN_WIDTH; x += mountain_width) {
        DrawTextureRec(gameState->spritesheet, src_rect,
                       (Vector2){(float)x, (float)mountain_y}, WHITE);
    }
    for (int x = (int)mountain_x - mountain_width; x > -mountain_width;
         x -= mountain_width) {
        DrawTextureRec(gameState->spritesheet, src_rect,
                       (Vector2){(float)x, (float)mountain_y}, WHITE);
    }
...
static int get_car_texture_index(GameState* gameState, bool is_night_stage,
                                 int bg_index, Car* car) {
    if (is_night_stage) {
        return (bg_index == 13)
...
void render_enemy_cars(GameState* gameState, Enduro* env) {
    ...
    Rectangle clip_rect = {(float)PLAYABLE_AREA_LEFT, clip_start_y,
                           (float)(PLAYABLE_AREA_RIGHT - PLAYABLE_AREA_LEFT),
                           clip_height};
    BeginScissorMode(clip_rect.x, clip_rect.y, clip_rect.width,
                     clip_rect.height);
    for (int i = 0; i < env->num_enemies; i++) {
        Car* car = &env->enemy_cars[i];
        ...
        int car_asset_index =
            get_car_texture_index(gameState, is_night_stage, bg_index, car);
        Rectangle src_rect = asset_map[car_asset_index];
        float car_center_x = car_x_in_lane(env, car->lane, car->y);
        ...
        DrawTexturePro(gameState->spritesheet, src_rect,
                       (Rectangle){car_x, car_y, src_rect.width * car_scale,
                                   src_rect.height * car_scale},
                       (Vector2){0, 0}, 0.0f, WHITE);
    }
    EndScissorMode();
}
...
void render_road(GameState* gameState, Enduro* env) {
    ...
    for (float y = road_start_y; y <= PLAYABLE_AREA_BOTTOM; y += 0.75f) {
        float adjusted_y =
            (env->speed < 0) ? y
                             : y + (float)fmod(env->road_scroll_offset, 0.75f);
