...
typedef struct GameState {
  ...
  float score_digit_offsets[SCORE_DIGITS];  // Offset for scrolling effect for
                                            // each digit
  int score_digit_currents[SCORE_DIGITS];   // Current digit being displayed for
                                            // each position
...
typedef struct Enduro {
  ...
  float
      collision_cooldown_car_vs_road;  // Timer for car vs road edge collisions
  int drift_direction;  // Which way player car drifts whilst nooped after crash
                        // vs car
...
void update_road_curve(Enduro* env) {
  ...
  if (env->current_vanishing_point_x < env->target_vanishing_point_x) {
    env->current_vanishing_point_x =
        fminf(env->current_vanishing_point_x + vanishing_point_transition_speed,
              env->target_vanishing_point_x);
  } else if (env->current_vanishing_point_x > env->target_vanishing_point_x) {
    env->current_vanishing_point_x =
        fmaxf(env->current_vanishing_point_x - vanishing_point_transition_speed,
              env->target_vanishing_point_x);
  }
...
void compute_observations(Enduro* env) {
  ...
  for (int i = 0; i < MAX_ENEMIES; i++) {
    ...
    if (car->y > VANISHING_POINT_Y && car->y < PLAYABLE_AREA_BOTTOM) {
      ...
      float delta_x_norm =
          (car->last_x - car->x) / (env->last_road_right - env->last_road_left);
...
void render_scoreboard(GameState* gameState, Enduro* env) {
  ...
  for (int i = 0; i < SCORE_DIGITS; ++i) {  // Draw scrolling score digits
    ...
    if (gameState->score_digit_scrolling[i]) {
      ...
      Rectangle dest_rect_current = {
          digit_x, (float)(score_start_y + (int)offset), (float)digit_width,
      ...
      DrawTexturePro(gameState->spritesheet, src_rect_next, dest_rect_next,
  ...
  if (env->day_completed) {
    ...
    Rectangle dest_rect = {(float)cars_x, (float)cars_y, flag_src_rect.width,
                           flag_src_rect.height};
...
void render_road(GameState* gameState, Enduro* env) {
  ...
  for (float y = road_start_y; y <= PLAYABLE_AREA_BOTTOM; y += 0.75f) {
    float adjusted_y =
        (env->speed < 0) ? y : y + (float)fmod(env->road_scroll_offset, 0.75f);
