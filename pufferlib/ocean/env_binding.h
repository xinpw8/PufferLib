...
//     // printf("[DEBUG VEC_LOG] Called with %d envs\n", vec->num_envs);
...
//     // Debug: Print some log values before aggregation
//     // for (int i = 0; i < vec->num_envs && i < 3; i++) {  // Just print first 3 envs
//     //     Env* env = vec->envs[i];
//     //     printf("[DEBUG VEC_LOG] Env %d log.n=%f, log.episode_return=%f, log.episode_length=%f\n",
//     //            i, env->log.n, env->log.episode_return, env->log.episode_length);
//     // }
...
//     // printf("[DEBUG VEC_LOG] Aggregate n=%f, episode_return=%f, episode_length=%f\n",
//     //        aggregate.n, aggregate.episode_return, aggregate.episode_length);

//     PyObject* dict = PyDict_New();
//     if (aggregate.n == 0.0f) {
//         // printf("[DEBUG VEC_LOG] No episodes ended (n=0), returning empty dict without resetting\n");
//         // Still need to reset logs to prevent accumulation
//         for (int i = 0; i < vec->num_envs; i++) {
...
//     // printf("[DEBUG VEC_LOG] After averaging: episode_return=%f, episode_length=%f\n",
//     //        aggregate.episode_return, aggregate.episode_length);

//     // User populates dict
//     my_log(dict, &aggregate);
//     assign_to_dict(dict, "n", n);

//     // Reset logs after we've collected and are returning data
//     // printf("[DEBUG VEC_LOG] Resetting logs for all %d envs\n", vec->num_envs);
//     for (int i = 0; i < vec->num_envs; i++) {
//         memset(&vec->envs[i]->log, 0, sizeof(Log));
//     }
...
static PyObject *env_init(PyObject *self, PyObject *args, PyObject *kwargs) {
  ...
  if (PyArray_ITEMSIZE(actions) == sizeof(double)) {
    PyErr_SetString(PyExc_ValueError,
                    "Action tensor passed as float64 (pass np.float32 buffer)");
    return NULL;
  }
  ...
  if (kwargs == NULL) {
    ...
    Py_INCREF(kwargs); // We need to increment the reference since we'll be
                       // modifying it
  }
...
static PyObject *vec_init(PyObject *self, PyObject *args, PyObject *kwargs) {
  ...
  if (PyArray_NDIM(observations) < 2) {
    PyErr_SetString(PyExc_ValueError,
                    "Batched Observations must be at least 2D");
    return NULL;
  }
  ...
  printf("[VEC_INIT DEBUG] Creating %d environments\n", num_envs);
  for (int i = 0; i < num_envs; i++) {
    Env *env = (Env *)calloc(1, sizeof(Env));
    if (!env) {
      PyErr_SetString(PyExc_MemoryError, "Failed to allocate environment");
    ...
    env->rewards = (void *)((char *)PyArray_DATA(rewards) +
                            i * PyArray_STRIDE(rewards, 0));
    env->terminals = (void *)((char *)PyArray_DATA(terminals) +
                              i * PyArray_STRIDE(terminals, 0));
    // env->truncations = (void*)((char*)PyArray_DATA(truncations) +
    // i*PyArray_STRIDE(truncations, 0));
  ...
  for (int i = 0; i < num_envs; i++) {
    ...
    if (!PyObject_TypeCheck(handle_obj, &PyLong_Type)) {
      PyErr_SetString(PyExc_TypeError,
                      "Env ids must be integers. Pass them as separate args "
                      "with *env_ids, not as a list.");
      return NULL;
    }
    vec->envs[i] = (Env *)PyLong_AsVoidPtr(handle_obj);
  }
...
static double unpack(PyObject *kwargs, char *key) {
  ...
  if (PyLong_Check(val)) {
    ...
    if (out > INT_MAX || out < INT_MIN) {
      char error_msg[100];
      snprintf(error_msg, sizeof(error_msg),
               "Value %ld of integer argument %s is out of range", out, key);
      PyErr_SetString(PyExc_TypeError, error_msg);
      return 1;
    }
    // Cast on return. Safe because double can represent all 32-bit ints exactly
  ...
  char error_msg[100];
  snprintf(error_msg, sizeof(error_msg), "Failed to unpack keyword %s as int",
           key);
  PyErr_SetString(PyExc_TypeError, error_msg);
  return 1;
}
...
static PyMethodDef methods[] = {
    {"env_init", (PyCFunction)env_init, METH_VARARGS | METH_KEYWORDS,
     "Init environment with observation, action, reward, terminal, truncation "
     "arrays"},
    {"env_reset", env_reset, METH_VARARGS, "Reset the environment"},
    {"env_step", env_step, METH_VARARGS, "Step the environment"},
    {"env_render", env_render, METH_VARARGS, "Render the environment"},
    {"env_close", env_close, METH_VARARGS, "Close the environment"},
    {"env_get", env_get, METH_VARARGS, "Get the environment state"},
    {"env_put", (PyCFunction)env_put, METH_VARARGS | METH_KEYWORDS,
     "Put stuff into env"},
    {"vectorize", vectorize, METH_VARARGS,
     "Make a vector of environment handles"},
    {"vec_init", (PyCFunction)vec_init, METH_VARARGS | METH_KEYWORDS,
     "Initialize a vector of environments"},
    {"vec_reset", vec_reset, METH_VARARGS, "Reset the vector of environments"},
    {"vec_step", vec_step, METH_VARARGS, "Step the vector of environments"},
    {"vec_log", vec_log, METH_VARARGS, "Log the vector of environments"},
    {"vec_render", vec_render, METH_VARARGS,
     "Render the vector of environments"},
    {"vec_close", vec_close, METH_VARARGS, "Close the vector of environments"},
    {"shared", (PyCFunction)my_shared, METH_VARARGS | METH_KEYWORDS,
     "Shared state"},
    MY_METHODS,
    {NULL, NULL, 0, NULL}};

// Module definition
static PyModuleDef module = {PyModuleDef_HEAD_INIT, "binding", NULL, -1,
                             methods};
