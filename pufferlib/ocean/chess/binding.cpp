...
static PyObject *print_profile(PyObject *self, PyObject *args);
...
#define MY_METHODS                                                             \
  {"env_set_self_play", env_set_self_play, METH_VARARGS,                       \
   "Enable self-play mode"},                                                   \
      ...
      {"print_profile", print_profile, METH_NOARGS,                            \
       "Prints C++ profiling data"},                                           \
  {                                                                            \
    NULL, NULL, 0, NULL                                                        \
  }
...
static int my_init(Env *env, PyObject *args, PyObject *kwargs) {
  ...
  env->reward_agent_captures_enemy_piece =
      unpack(kwargs, "reward_agent_captures_enemy_piece");
  env->reward_enemy_captures_agent_piece =
      unpack(kwargs, "reward_enemy_captures_agent_piece");
  ...
  env->reward_material_diff_white =
      unpack(kwargs, "reward_material_diff_white");
  env->reward_material_diff_black =
      unpack(kwargs, "reward_material_diff_black");
  env->max_depth = (int)unpack(kwargs, "max_depth");
  env->debug_disable_mask =
      (bool)unpack(kwargs, "debug_disable_mask"); // FIX: Pass debug flag to C++
  init(env);
  return 0;
}
...
static int my_log(PyObject *dict, Log *log) {
  ...
  assign_to_dict(dict, "reward_agent_captures_enemy_piece",
                 log->reward_agent_captures_enemy_piece);
  assign_to_dict(dict, "reward_enemy_captures_agent_piece",
                 log->reward_enemy_captures_agent_piece);
...
extern "C" void c_print_profile_data();

// Add the Python wrapper function
static PyObject *print_profile(PyObject *self, PyObject *args) {
  c_print_profile_data();
  Py_RETURN_NONE;
}
// // binding.c
// #pragma once
...
//     {"print_profile", print_profile, METH_NOARGS, "Prints C++ profiling data"}, \
//     {NULL, NULL, 0, NULL}
