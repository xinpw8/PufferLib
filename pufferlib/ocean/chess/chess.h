...
#define MAX_PUZZLE_SET_SIZE 20
  ...
  char puzzle_set_solutions[MAX_PUZZLE_SET_SIZE][10]
                           [6]; // Solutions for each puzzle
  ...
  // Per-puzzle statistics tracking
  int puzzle_attempts_per_puzzle[MAX_PUZZLE_SET_SIZE];  // Attempts per puzzle
  int puzzle_successes_per_puzzle[MAX_PUZZLE_SET_SIZE]; // Successes per puzzle

  // Per-puzzle move caching to avoid conflicts when switching puzzles
  char puzzle_white_moves_cache[MAX_PUZZLE_SET_SIZE][256][6]; // White moves for each puzzle
  int puzzle_white_moves_count[MAX_PUZZLE_SET_SIZE];          // White move counts
  int puzzle_white_action_ids[MAX_PUZZLE_SET_SIZE][256];      // White action IDs
  char puzzle_black_moves_cache[MAX_PUZZLE_SET_SIZE][256][6]; // Black moves for each puzzle  
  int puzzle_black_moves_count[MAX_PUZZLE_SET_SIZE];          // Black move counts
  int puzzle_black_action_ids[MAX_PUZZLE_SET_SIZE][256];      // Black action IDs
  bool puzzle_moves_cached[MAX_PUZZLE_SET_SIZE];              // Whether moves are cached for each puzzle
...
static void add_legal_move(ChessContext *ctx, LegalMoves *moves,
  ...
  if (move.from.x == 5 && move.from.y == 6 && move.to.x == 5 &&
      move.to.y == 7) {
  ...
  if (chess_is_legal_move(ctx, move)) {
    ...
    if (move.from.x == 5 && move.from.y == 6 && move.to.x == 5 &&
        move.to.y == 7) {
      //       printf("[F7F8 DEBUG] *** f7f8 PASSED legality check! Adding to
      //       moves list at index %d ***\n", moves->count);
      fflush(stdout);
    }
    ...
    if (move.from.x == 5 && move.from.y == 6 && move.to.x == 5 &&
        move.to.y == 7) {
      //       // printf("[F7F8 DEBUG] XXX f7f8 FAILED legality check! NOT added
      //       to moves XXX\n");
      fflush(stdout);
    }
  }
  ...
  case QUEEN: {
    ...
    if (piece->type == ROOK) {
      ...
      if (piece->type == QUEEN && from.x == 5 && from.y == 6) {
        //         // printf("[F7F8 DEBUG] Generating moves for Queen on f7
        //         (square 53)\n");
        fflush(stdout);
      }
    }
    for (int d = 0; d < num_dirs; d++) {
      ...
      for (int dist = 1; dist < 8; dist++) {
        ...
        // DEBUG: Check for f7f8 move
        if (piece->type == QUEEN && from.x == 5 && from.y == 6 && to.x == 5 &&
            to.y == 7) {
          //           printf("[F7F8 DEBUG] Found f7f8 candidate: Queen from
        ...
        if (target && target->type != EMPTY) {
          if (target->color == them) {
            ...
            if (piece->type == QUEEN && from.x == 5 && from.y == 6 &&
                to.x == 5 && to.y == 7) {
              //               printf("[F7F8 DEBUG] f7f8 is a CAPTURE (target:
              //               type=%d, color=%d). Calling add_legal_move...\n",
  ...
  for (int i = 0; i < moves.count && i < 256; i++) {
    ...
    if (current_player == C_WHITE) {
      ...
      if (strcmp(uci_str, "f7f8") == 0) {
        // printf("[F7F8 ACTION] f7f8 mapped to action ID %d (stored at
        // moves[%d])\n",
...
static void compute_single_agent_observation(CChess *env, ChessContext *ctx,
  ...
  if (false && ctx->observation_cached &&
    ...
    //     // printf("[COMPUTE_OBS] Starting compute_single_agent_observation
    //     for player %s at offset %d\n",
    ...
    for (int y_white_perspective = 0; y_white_perspective < 8;
      ...
      for (int x = 0; x < 8; x++) {
        ...
        if (p->type == EMPTY) {
          ...
          if (piece_count <= 3) {
            //             // printf("[OBS VERIFY] Just wrote
            //             env->observations[%d] = 1.0, readback = %.1f\n",
          ...
          if (piece_count <= 5) {
            ...
            //             // printf("[OBS ENCODE] Piece %d: %s %s at square %d
            //             (x=%d,y=%d) -> obs[%d]=1.0\n",
            //                    piece_count,
          ...
          if (p->type == QUEEN &&
              square_index_actual == 53) { // f7 = 6*8+5 = 53
            //             // printf("[OBS DEBUG] Found Queen on f7 (square %d),
            //             set obs[%d] = 1.0\n",
            //                    square_index_actual,
            //                    obs_offset + (plane_offset + piece_plane) * 64
            //                    + obs_square_idx);
            fflush(stdout);
          }
        }
    ...
    if (piece_count < 10) {
      //       // printf("[OBS DEBUG] WARNING: Only %d pieces found on
      //       board!\n", piece_count);
      fflush(stdout);
    }
  ...
  if (env->debug_disable_mask) {
    // Debug mode: All moves are legal (first 64 actions for sparse
    // representation)
  ...
  // DEBUG: Removed orphaned FEN printing code that was causing compilation
  // errors
  /*
      ...
      if (piece_a2->type == EMPTY) {
        ...
        const char* color_str = (piece_a2->color == C_WHITE) ? "White" :
    "Black";
        // printf("%s %s\n", color_str, type_str);
      }
...
void validate_chess_observation_integrity(CChess *env, ChessContext *ctx,
  ...
  if (board_sum < 1.0f) {
    ...
    // printf(" %s observation at offset %d: board_sum=%.3f
    // total_legal_actions=%d\n",
  ...
  if (board_sum < 10.0f) {
    // printf("[CHESS_FATAL] Board sum is %.1f - board appears
    // uninitialized!\n",
    //        board_sum);
    // printf(" This suggests the environment reset is not working
    // properly.\n");
    exit(1);
  }
...
void compute_observation_with_perspective(CChess *env, ChessContext *ctx) {
  ...
  int nonzero_count = 0;
  for (int i = 0; i < 1537;
       i++) { // Check ALL observation values including action mask
  ...
  //   // printf("[OBS BUFFER] Key indices: obs[22]=%.1f obs[136]=%.1f
  //   obs[391]=%.1f obs[1472]=%.1f\n",
  //          env->observations[22], env->observations[136],
  //          env->observations[391], env->observations[1472]);
...
static int global_total_puzzle_attempts = 0;
...
void init(CChess *env) {
  ...
  // CRITICAL: Explicitly initialize episode-specific puzzle stats to ensure
  // they start at 0
  ...
  // Initialize per-puzzle move caches
  for (int i = 0; i < MAX_PUZZLE_SET_SIZE; i++) {
    env->context.puzzle_moves_cached[i] = false;
    env->context.puzzle_white_moves_count[i] = 0;
    env->context.puzzle_black_moves_count[i] = 0;
  }
  ...
  env->context.dual_agent_self_play_mode =
      false; // Default to single-agent mode
  ...
  // Initialize per-puzzle statistics
  for (int i = 0; i < MAX_PUZZLE_SET_SIZE; i++) {
    env->context.puzzle_attempts_per_puzzle[i] = 0;
    env->context.puzzle_successes_per_puzzle[i] = 0;
  }
...
bool parse_fen(const char *fen, ChessBoard *board) {
  ...
  if (fen[i] == '-') {
    ...
    while (fen[i] && fen[i] != ' ') {
      switch (fen[i]) {
      case 'K':
        board->castle_rights |= 0x01;
        break; // White kingside
      case 'Q':
        board->castle_rights |= 0x02;
        break; // White queenside
      case 'k':
        board->castle_rights |= 0x04;
        break; // Black kingside
      case 'q':
        board->castle_rights |= 0x08;
...
void set_puzzle_data(CChess *env, const char *fen, const char *solution_moves[],
  ...
  // USE ACTUAL PUZZLE DATA FROM PYTHON (not hardcoded debug values)
  // Reset tries counter when loading new puzzle
  env->context.puzzle_tries_this_env = 0;

  // Store puzzle FEN - USE ACTUAL FEN PASSED FROM PYTHON
  strncpy(env->context.puzzle_fen, fen,
          sizeof(env->context.puzzle_fen) - 1);
  env->context.puzzle_fen[sizeof(env->context.puzzle_fen) - 1] = '\0';

  // Store solution moves - USE ACTUAL SOLUTION FROM PYTHON
  env->context.puzzle_solution_length = solution_length;
  for (int i = 0; i < solution_length && i < 10; i++) {  // Max 10 solution moves
  ...
  //           env->context.puzzle_board_cache.board[8].color == C_WHITE) ?
  //           "WHITE ROOK" : "EMPTY/OTHER");
...
void set_puzzle_set(CChess *env, int num_puzzles, const char **fens,
  ...
  // Set the puzzle set size from Python
  env->context.puzzle_set_size = num_puzzles;
  printf("[C++ PUZZLE SET] Env %d - Setting puzzle_set_size to %d\n", env->env_id, num_puzzles);
  
  // CRITICAL: Clear all cached moves when loading new puzzles
  // This prevents using stale cached moves from previous puzzle sets
  for (int i = 0; i < MAX_PUZZLE_SET_SIZE; i++) {
    env->context.puzzle_moves_cached[i] = false;
    env->context.puzzle_white_moves_count[i] = 0;
    env->context.puzzle_black_moves_count[i] = 0;
  ...
  // Store all puzzles from Python (not hardcoded)
  for (int i = 0; i < num_puzzles && i < MAX_PUZZLE_SET_SIZE; i++) {
    ...
    for (int j = 0; j < solution_lengths[i] && j < 10; j++) {
  ...
  if (env->context.puzzle_set_size > 0) {
    ...
    for (int i = 0; i < env->context.puzzle_set_solution_lengths[0] && i < 10;
         i++) {
      first_solution[i] = env->context.puzzle_set_solutions[0][i];
    }
    set_puzzle_data(env, env->context.puzzle_set_fens[0], first_solution,
...
void set_puzzle_training_params(CChess *env, int max_tries_per_env,
                                float success_threshold) {
  //   // printf("[PUZZLE PARAMS] Setting max_tries_per_env = %d,
  //   success_threshold = %.2f\n",
...
void c_reset(CChess *env) {
  ...
  } else if (env->context.puzzle_mode && env->context.puzzle_set_size > 0 &&
             strlen(env->context.puzzle_fen) > 0) {
  ...
  if (env->context.puzzle_mode) {
    // Randomly select a puzzle from the loaded set
    if (env->context.puzzle_set_size > 1) {
      int new_idx = rand() % env->context.puzzle_set_size;
      
      printf("[PUZZLE SELECT] Env %d Reset selecting puzzle %d (was %d, set size: %d)\n",
             env->env_id, new_idx, env->context.current_puzzle_idx, env->context.puzzle_set_size);
      
      // Debug: Always show which puzzle's FEN and solution we're about to use
      if (new_idx < env->context.puzzle_set_size) {
        printf("[PUZZLE DEBUG] Env %d - Puzzle %d - FEN: %.50s... Solution: %s\n",
               env->env_id, new_idx, env->context.puzzle_set_fens[new_idx],
               env->context.puzzle_set_solutions[new_idx][0]);
      }
      
      // ALWAYS reload the puzzle data to ensure correct board state
      // (Previously we skipped if same puzzle, causing board state issues)
      env->context.current_puzzle_idx = new_idx;
      
      // Create temporary array of solution pointers
      const char *new_solution[10];
      for (int i = 0;
           i < env->context.puzzle_set_solution_lengths[new_idx] && i < 10;
           i++) {
      ...
      // Parse the puzzle FEN and cache it
      if (!parse_fen(env->context.puzzle_fen, &env->context.board)) {
        printf("[ERROR] Failed to parse puzzle %d FEN: %s\n", new_idx, env->context.puzzle_fen);
        init_board(&env->context.board);
      }
      env->context.puzzle_board_cache = env->context.board;
      env->context.puzzle_board_cached = true;
      
      // Check if we have cached moves for this puzzle
      if (env->context.puzzle_moves_cached[new_idx]) {
        // Restore cached moves for this specific puzzle
        memcpy(env->context.white_legal_moves_buffer, 
               env->context.puzzle_white_moves_cache[new_idx],
               sizeof(env->context.white_legal_moves_buffer));
        env->context.white_legal_moves_count = env->context.puzzle_white_moves_count[new_idx];
        memcpy(env->context.white_legal_action_ids,
               env->context.puzzle_white_action_ids[new_idx],
               sizeof(env->context.white_legal_action_ids));
        
        memcpy(env->context.black_legal_moves_buffer,
               env->context.puzzle_black_moves_cache[new_idx],
               sizeof(env->context.black_legal_moves_buffer));
        env->context.black_legal_moves_count = env->context.puzzle_black_moves_count[new_idx];
        memcpy(env->context.black_legal_action_ids,
               env->context.puzzle_black_action_ids[new_idx],
               sizeof(env->context.black_legal_action_ids));
        
        // Mark caches as valid
        env->context.white_moves_cached = true;
        env->context.black_moves_cached = true;
        env->context.position_fully_cached = true;
        env->context.cached_board_hash = env->context.board.zobrist_hash;
        
        int num_legal = (env->context.board.to_move == C_WHITE) ? 
                        env->context.white_legal_moves_count : 
                        env->context.black_legal_moves_count;
        printf("[PUZZLE CACHE HIT] Env %d - Puzzle %d loaded from cache - %d legal moves, expected: %s\n",
               env->env_id, new_idx, num_legal, env->context.puzzle_solution[0]);
        
        // Debug: Print first 3 cached moves for puzzle 0
        if (new_idx == 0 && env->context.board.to_move == C_WHITE) {
          printf("  Cached moves for puzzle 0: ");
          for (int i = 0; i < 3 && i < env->context.white_legal_moves_count; i++) {
            printf("%s ", env->context.white_legal_moves_buffer[i]);
          }
          printf("\n");
        }
      } else {
        // No cache, need to generate moves and cache them
        env->context.white_moves_cached = false;
        env->context.black_moves_cached = false;
        env->context.position_fully_cached = false;
        ...
        // Cache the moves for this puzzle
        memcpy(env->context.puzzle_white_moves_cache[new_idx],
               env->context.white_legal_moves_buffer,
               sizeof(env->context.white_legal_moves_buffer));
        env->context.puzzle_white_moves_count[new_idx] = env->context.white_legal_moves_count;
        memcpy(env->context.puzzle_white_action_ids[new_idx],
               env->context.white_legal_action_ids,
               sizeof(env->context.white_legal_action_ids));
        
        memcpy(env->context.puzzle_black_moves_cache[new_idx],
               env->context.black_legal_moves_buffer,
               sizeof(env->context.black_legal_moves_buffer));
        env->context.puzzle_black_moves_count[new_idx] = env->context.black_legal_moves_count;
        memcpy(env->context.puzzle_black_action_ids[new_idx],
               env->context.black_legal_action_ids,
               sizeof(env->context.black_legal_action_ids));
        
        env->context.puzzle_moves_cached[new_idx] = true;
        
        int num_legal = (env->context.board.to_move == C_WHITE) ? 
                        env->context.white_legal_moves_count : 
                        env->context.black_legal_moves_count;
        printf("[PUZZLE CACHE MISS] Env %d - Puzzle %d generated and cached - %d legal moves, expected: %s\n",
               env->env_id, new_idx, num_legal, env->context.puzzle_solution[0]);
               
        // Debug: Print first 3 generated moves for puzzle 0
        if (new_idx == 0 && env->context.board.to_move == C_WHITE) {
          printf("  Generated moves for puzzle 0: ");
          for (int i = 0; i < 3 && i < env->context.white_legal_moves_count; i++) {
            printf("%s ", env->context.white_legal_moves_buffer[i]);
          }
          printf("\n");
  ...
  // In puzzle mode, moves are already cached or generated earlier
  // For non-puzzle mode, we need to generate them fresh
  if (!env->context.puzzle_mode) {
    // CRITICAL: Invalidate all caches before generating moves for the new board
    env->context.white_moves_cached = false;
    env->context.black_moves_cached = false;
    env->context.position_fully_cached = false;
    env->context.cached_board_hash = 0;
  ...
  //   // printf("[RESET OBS CHECK] After reset, obs[22]=%.1f obs[136]=%.1f
  //   obs[391]=%.1f\n",
  //          env->observations[22], env->observations[136],
  //          env->observations[391]);
...
void c_step(CChess *env) {
  ...
  //   printf("\n[C_STEP %d START] terminals[0]=%d, puzzle_failed=%d,
  //   puzzle_completed=%d\n",
  //          step_count, env->terminals[0], env->context.puzzle_failed,
  //          env->context.puzzle_completed);
  //   printf("  Puzzle FEN: %.50s...\n", env->context.puzzle_fen ?
  //   env->context.puzzle_fen : "NULL");
  ...
  if (env->terminals && (env->terminals[0] || env->terminals[1])) {
    // printf("[C_STEP %d] Episode terminated (terminals[0]=%d), resetting for
    // next episode\n",
    //        step_count, env->terminals[0]);
    c_reset(env); // Reset the environment for the next episode
    // After reset, terminals should be 0, so we can continue with the new
    // episode
  }
  ...
  if (env->context.step_count % 1 == 0) {
    ...
    // printf("[ENV %3d] step=%4d | solve_rate=%5.1f%% | tries=%3d |
    // solves=%3d\n",
  ...
  // REMOVED: Puzzle reset logic should ONLY happen in c_reset()
  // This was causing timing issues and observation/action mismatches
  ...
  //   printf("[TURN] %s to move\n", env->context.board.to_move == C_WHITE ?
  //   "WHITE" : "BLACK"); printf("[LEGAL MOVES] White: %.0f, Black: %.0f\n",
  ...
  if (!is_action_legal) {
    ...
    if (env->context.puzzle_mode) {
      ...
      env->context.puzzle_attempts_per_puzzle[env->context.current_puzzle_idx]++;
      
      // Log the failed attempt with detailed info
      float puzzle_success_rate = 0.0f;
      if (env->context.puzzle_attempts_per_puzzle[env->context.current_puzzle_idx] > 0) {
        puzzle_success_rate = (float)env->context.puzzle_successes_per_puzzle[env->context.current_puzzle_idx] / 
                             env->context.puzzle_attempts_per_puzzle[env->context.current_puzzle_idx];
      }
      float total_success_rate = 0.0f;
      if (env->global_puzzle_attempts > 0) {
        total_success_rate = (float)env->global_puzzle_successes / env->global_puzzle_attempts;
      }
      printf("[PUZZLE ATTEMPT] Env %d | Puzzle %d | Action: %d (%s) | Result: FAILED | This puzzle: %.1f%% (%d/%d) | Total: %.1f%% (%d/%d)\n",
             env->env_id, env->context.current_puzzle_idx, action_idx, uci_move_canonical,
             puzzle_success_rate * 100.0f, 
             env->context.puzzle_successes_per_puzzle[env->context.current_puzzle_idx],
             env->context.puzzle_attempts_per_puzzle[env->context.current_puzzle_idx],
             total_success_rate * 100.0f,
             env->global_puzzle_successes, env->global_puzzle_attempts);

      // Check if we've reached the configured puzzle tries limit for the
      // episode
      if (env->context.puzzle_tries_this_episode >=
          env->context.puzzle_max_tries_per_env) {
        env->terminals[0] = 1;
        env->terminals[1] = 1;
        add_log(env);
        // // printf("[OBS COMPUTE] Line 7679: Puzzle episode termination (max
        // tries)\n");
        compute_observation_with_perspective(env, &env->context);
        return;
      }

      // Always terminate on illegal move in puzzle mode
      env->terminals[0] = 1;
      env->terminals[1] = 1;
...
//     // printf("[PUZZLE STEP] step_count=%d, wrong_moves=%.0f, solves=%d,
tries=%d\n", env->context.step_count,
  ...
  */
  ...
  // Episode termination is now handled where puzzle_tries_this_episode is
  // incremented This ensures episodes terminate immediately after the
  // configured number of puzzle attempts
  ...
  if (env->context.puzzle_mode) {
    ...
    if (debug_step_count <= 5) {
      ...
      //   printf("\n");
      //   printf("  Mask at 1472: count=%.0f, first 5 actions: ",
      //   env->observations[1472]);
  ...
  if (move_applied && env->context.puzzle_mode) {
    ...
    if (env->context.puzzle_move_index < env->context.puzzle_solution_length) {
      ...
      //       // printf("[PUZZLE DEBUG] Action %d converted to UCI: %s,
      //       Expected: %s\n",
      //              action_idx, uci_move_canonical, expected_move);

      // COMBINED FIX: Increment counters for EVERY attempt, BEFORE checking
      // correctness
      env->context.puzzle_attempts_this_episode += 1.0f;
      env->global_puzzle_attempts++;  // Track global attempt
      env->context.puzzle_attempts_per_puzzle[env->context.current_puzzle_idx]++; // Track per-puzzle attempt
      ...
      // // printf("[PUZZLE TRAIN] Env %d | Puzzle idx: %d | Tries: %d | FEN: %s
      // | Expected: %s | Agent chose: %s | Action: %d | ",
      //        env->env_id, env->context.current_puzzle_idx,
      //        env->context.puzzle_tries_this_env, env->context.puzzle_fen,
      //        expected_move, uci_move_canonical, action_idx);
      ...
      // printf("Obs[22]: %.1f | Obs[136]: %.1f | Obs[391]: %.1f | NonZero:
      // %d/1472 | ",
      //        env->observations[22], env->observations[136],
      //        env->observations[391], num_nonzero);
      ...
      if (cmp_result == 0) {
        ...
        //         // printf("[PUZZLE DEBUG] CORRECT MOVE! Agent idx=%d, Adding
        //         rewards: correct=%f, solved=%f\n",
        //                agent_idx, env->reward_correct_move,
        ...
        //         // printf("[PUZZLE DEBUG] After increment: move_index=%d,
        //         solution_length=%d\n",
        //                env->context.puzzle_move_index,
        //                env->context.puzzle_solution_length);
        if (env->context.puzzle_move_index >=
          ...
          //           // printf("[PUZZLE DEBUG] PUZZLE SOLVED! Adding solved
          //           reward: %f\n", env->reward_puzzle_solved);
          env->rewards[agent_idx] += env->reward_puzzle_solved;
          //           // printf("[PUZZLE DEBUG] Final reward: env->rewards[%d]
          //           = %f\n", agent_idx, env->rewards[agent_idx]);
          ...
          // Attempts already counted above before checking correctness
          env->context.puzzle_successes_per_puzzle[env->context.current_puzzle_idx]++;

          // Calculate per-puzzle and total success rates
          float puzzle_success_rate = (float)env->context.puzzle_successes_per_puzzle[env->context.current_puzzle_idx] / 
                                      env->context.puzzle_attempts_per_puzzle[env->context.current_puzzle_idx];
          float total_success_rate = (float)env->global_puzzle_successes / env->global_puzzle_attempts;
          
          // Log the successful attempt with detailed stats
          printf("[PUZZLE ATTEMPT] Env %d | Puzzle %d | Action: %d (%s) | Result: SUCCESS | This puzzle: %.1f%% (%d/%d) | Total: %.1f%% (%d/%d)\n",
                 env->env_id, env->context.current_puzzle_idx, action_idx, uci_move_canonical,
                 puzzle_success_rate * 100.0f, 
                 env->context.puzzle_successes_per_puzzle[env->context.current_puzzle_idx],
                 env->context.puzzle_attempts_per_puzzle[env->context.current_puzzle_idx],
          ...
          // Log which puzzle was successfully solved
          printf("[PUZZLE SOLVED] Env %d - Puzzle %d solved (set size: %d)\n", env->env_id, 
                 env->context.current_puzzle_idx, env->context.puzzle_set_size);
          ...
          // IMPORTANT: DO NOT reset board - agent needs to see the RESULT of
          // their move The agent needs to see the POST-MOVE board state with
          // the reward signal
          // // printf("[PUZZLE SOLVED] Keeping post-move board state for
          // observation\n"); env->context.board already has the move applied -
          // keep it!
          ...
          env->terminals[1] = 1;
          add_log(env);
          // // // printf("[OBS COMPUTE] Line 7903: After correct move -
          // computing obs with POST-MOVE state\n");
        ...
        // printf("[DEBUG] env->reward_puzzle_failed = %.4f, total_penalty =
        // %.4f\n",
        ...
        // env->rewards[agent_idx]); In puzzle mode, only white plays, so only
        // update white's episode return
        ...
        // Attempts already tracked above in puzzle mode section before checking correctness
        
        // Log this wrong puzzle attempt
        float puzzle_success_rate = 0.0f;
        if (env->context.puzzle_attempts_per_puzzle[env->context.current_puzzle_idx] > 0) {
          puzzle_success_rate = (float)env->context.puzzle_successes_per_puzzle[env->context.current_puzzle_idx] / 
                               env->context.puzzle_attempts_per_puzzle[env->context.current_puzzle_idx];
        ...
        printf("[PUZZLE ATTEMPT] Env %d | Puzzle %d | Action: %d (%s) | Result: WRONG | This puzzle: %.1f%% (%d/%d) | Total: %.1f%% (%d/%d)\n",
               env->env_id, env->context.current_puzzle_idx, action_idx, uci_move_canonical,
               puzzle_success_rate * 100.0f, 
               env->context.puzzle_successes_per_puzzle[env->context.current_puzzle_idx],
               env->context.puzzle_attempts_per_puzzle[env->context.current_puzzle_idx],
        ...
        // printf("[WRONG MOVE] wrong_moves_this_episode now: %.1f,
        // tries_this_env: %d\n",
        //        env->context.puzzle_wrong_moves_this_episode,
        //        env->context.puzzle_tries_this_env);
        // NOTE: global_puzzle_attempts already incremented above for ALL
        // attempts
        ...
        // Reset move index and terminate immediately
        env->context.puzzle_move_index = 0;
        ...
        // env->rewards[0]); printf("  puzzle_failed=%d\n",
        // env->context.puzzle_failed); printf("
        // puzzle_tries_this_episode=%d/%d\n",
        //        env->context.puzzle_tries_this_episode,
        //        env->context.puzzle_max_tries_per_env);
  ...
  if (game_over) {
    ...
    if (env->context.puzzle_mode) {
      //       // // printf("[OBS COMPUTE ERROR] Line 8308: SHOULD NOT REACH
      //       HERE IN PUZZLE MODE!\n");
      //       // printf("[OBS COMPUTE ERROR] Board state - rook at a2: %s\n",
      //              (env->context.board.board[8].type == ROOK &&
      //               env->context.board.board[8].color == C_WHITE) ? "YES" :
      //               "NO");
    }
  ...
  // fflush(stdout);
  // printf("[C_STEP EXIT] Final rewards: env->rewards[0]=%f,
  // env->rewards[1]=%f, rewards ptr=%p\n",
...
void add_log(CChess *env) {
  ...
  if (env->context.puzzle_mode && global_total_puzzle_attempts > 0) {
    float solve_rate =
        (float)global_total_puzzle_solves / (float)global_total_puzzle_attempts;
...
void update_puzzle_score(CChess *env) {
  if (env->context.puzzle_mode && global_total_puzzle_attempts > 0) {
    ...
    if (env->env_id == 0) {
      printf("[PUZZLE SCORE UPDATE] %d/%d solves, rate=%f, log.score=%f\n",
             global_total_puzzle_solves, global_total_puzzle_attempts,
             solve_rate, env->log.score);
    }
  }
}
