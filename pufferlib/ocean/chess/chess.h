...
typedef struct ChessContext {
  ...
  // DeepMind-style sequential training through shuffled dataset
  int puzzle_order[MAX_PUZZLE_SET_SIZE];  // Shuffled order of puzzle indices
  int puzzle_position;                     // Current position in puzzle_order array
  bool puzzle_order_initialized;           // Whether puzzle order has been initialized
...
// SYNCHRONIZED PUZZLE TRAINING: All envs work on same puzzle for clean BC
static int global_puzzle_index = 0;  // Shared across ALL environments
static int global_reset_counter = 0; // Track total resets across all envs
// IMPORTANT: Scale by number of environments for consistent training time per puzzle
// With 64 envs: 640 total resets = 10 resets per env before changing
// With 128 envs: 1280 total resets = 10 resets per env before changing
static int resets_per_puzzle = 640;  // Reduced for faster puzzle rotation
...
void init(CChess *env) {
  ...
  env->context.puzzle_order_initialized = false;  // Puzzle order not initialized yet
...
// PURE RANDOM SAMPLING: Simple and effective for supervised learning
// Each environment independently samples a random puzzle
// This is the standard approach for memorization tasks
...
if (env->env_id == 0 && total_resets % 1000 == 0) {
  printf("[RANDOM SAMPLING] %d total resets with %d envs, puzzle_set_size=%d\n", 
         total_resets, total_num_envs, env->context.puzzle_set_size);
}
}
