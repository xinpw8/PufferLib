...
void ChessBoard::GenerateRayDestinations_(Square sq, Color color,
  ...
  for (Square dest = sq + offset_step; InBoardArea(dest); dest += offset_step) {
    if (IsEmpty(dest)) {
      ...
      yield(dest);
      /* PUFFERLIB_FIX_RAY_TRACING_2025
       * Always stop tracing once the first occupied square is reached –
       * regardless of whether it is friendly or enemy.
       *
       * Rationale: In standard chess rules a sliding piece (rook, bishop,
       * queen) cannot "see through" ANY piece that stands between it and the
       * target square.  Allowing the ray to continue past an enemy piece can
       * cause incorrect attack detection; e.g. a queen was erroneously
       * considered to attack h4 through its own pawn on h3 in game
       * email_0103… at ply 131, marking the position as checkmate one move
       * early.  Terminating here fixes that class of bug while keeping the
       * original yield so that captures are still generated correctly.
       */
      break;
    } else {
