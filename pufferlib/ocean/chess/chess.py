...
class Chess(pufferlib.PufferEnv):
    ...
    def _load_puzzles(self):
        ...
        try:
            ...
            # Use a fixed seed for consistent puzzle selection during training
            import hashlib
            # Create a deterministic seed based on puzzle_set_size to ensure consistency
            seed_value = int(hashlib.md5(f"puzzle_{self.puzzle_set_size}".encode()).hexdigest()[:8], 16)
    ...
    def _step_puzzle_mode(self, actions):
        ...
        # DEBUG: Check what action is being sent
        if self.tick % 10 == 0:
            print(f"[PYTHON STEP] tick={self.tick}, action={actions[0] if len(actions) > 0 else 'NONE'}")
