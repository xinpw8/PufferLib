...
void _gelu(float *input, float *output, int size) {
  for (int i = 0; i < size; i++) {
    output[i] =
        0.5f * input[i] *
        (1 + tanhf(0.6628526501011142 *
                   (input[i] + 0.044715f * input[i] * input[i] * input[i])));
  }
}

float _sigmoid(float x);
inline float _sigmoid(float x) { return 1.0f / (1.0f + expf(-x)); }
...
void _linear_accumulate(float *input, float *weights, float *bias,
    ...
    for (int oc = 0; oc < out_channels; oc++) {
      for (int h = 0; h < h_out; h++) {
        for (int w = 0; w < w_out; w++) {
          int out_adr = (b * out_channels * h_out * w_out + oc * h_out * w_out +
                         +h * w_out + w);
          output[out_adr] = bias[oc];
          for (int ic = 0; ic < in_channels; ic++) {
            for (int kh = 0; kh < kernel_size; kh++) {
              for (int kw = 0; kw < kernel_size; kw++) {
                int in_adr = (b * in_channels * in_height * in_width +
                              ic * in_height * in_width +
                              (h * stride + kh) * in_width + (w * stride + kw));
                int weight_adr =
  ...
  for (int b = 0; b < batch_size; b++) {
    for (int oc = 0; oc < out_channels; oc++) {
      for (int d = 0; d < d_out; d++) {
        for (int h = 0; h < h_out; h++) {
          for (int w = 0; w < w_out; w++) {
            int out_adr = (b * out_channels * d_out * h_out * w_out +
                           oc * d_out * h_out * w_out + d * h_out * w_out +
                           h * w_out + w);
            output[out_adr] = bias[oc];
            for (int ic = 0; ic < in_channels; ic++) {
                  ...
                  for (int kw = 0; kw < kernel_size; kw++) {
                    ...
                    int weight_adr =
                        (oc * in_channels * kernel_size * kernel_size *
                         ...
                         kd * kernel_size * kernel_size + kh * kernel_size +
                         kw);
                    output[out_adr] += input[in_adr] * weights[weight_adr];
                  }
                }
...
void _lstm(float *input, float *state_h, float *state_c, float *weights_input,
  ...
  _linear(input, weights_input, bias_input, buffer, batch_size, input_size,
          4 * hidden_size);
  _linear_accumulate(state_h, weights_state, bias_state, buffer, batch_size,
  ...
  for (int b = 0; b < batch_size; b++) {
    ...
    for (int i = 0; i < hidden_size; i++) {
      ...
      state_h[hidden_offset + i] = (buffer[b_offset + 3 * hidden_size + i] *
                                    tanh(state_c[hidden_offset + i]));
    }
  }
}

void _embedding(int *input, float *weights, float *output, int batch_size,
                int num_embeddings, int embedding_dim) {
  for (int b = 0; b < batch_size; b++) {
    memcpy(output + b * embedding_dim, weights + input[b] * embedding_dim,
...
void _one_hot(int *input, int *output, int batch_size, int input_size,
              int num_classes) {
  for (int b = 0; b < batch_size; b++) {
    for (int i = 0; i < input_size; i++) {
      int in_adr = b * input_size + i;
      int out_adr =
          (b * input_size * num_classes + i * num_classes + input[in_adr]);
      output[out_adr] = 1.0f;
    }
  }
}

void _cat_dim1(float *x, float *y, float *output, int batch_size, int x_size,
               int y_size) {
  for (int b = 0; b < batch_size; b++) {
...
void _argmax_multidiscrete(float *input, int *output, int batch_size,
...
void _softmax_multidiscrete(float *input, int *output, int batch_size,
                            int logit_sizes[], int num_actions) {
  int in_adr = 0;
  for (int b = 0; b < batch_size; b++) {
    for (int a = 0; a < num_actions; a++) {
...
void _max_dim1(float *input, float *output, int batch_size, int seq_len,
               int feature_dim) {
  for (int b = 0; b < batch_size; b++) {
...
Linear *make_linear(Weights *weights, int batch_size, int input_dim,
                    int output_dim) {
...
void linear(Linear *layer, float *input) {
  _linear(input, layer->weights, layer->bias, layer->output, layer->batch_size,
...
void max_dim1(MaxDim1 *layer, float *input) {
  _max_dim1(input, layer->output, layer->batch_size, layer->seq_len,
            layer->feature_dim);
}
...
Conv2D *make_conv2d(Weights *weights, int batch_size, int in_width,
                    int in_height, int in_channels, int out_channels,
                    int kernel_size, int stride) {
  size_t buffer_size =
      batch_size * out_channels * in_height * in_width * sizeof(float);
...
void conv2d(Conv2D *layer, float *input) {
  _conv2d(input, layer->weights, layer->bias, layer->output, layer->batch_size,
          layer->in_width, layer->in_height, layer->in_channels,
...
Conv3D *make_conv3d(Weights *weights, int batch_size, int in_width,
                    int in_height, int in_depth, int in_channels,
                    int out_channels, int kernel_size, int stride) {

  size_t buffer_size = batch_size * out_channels * in_depth * in_height *
...
void conv3d(Conv3D *layer, float *input) {
  _conv3d(input, layer->weights, layer->bias, layer->output, layer->batch_size,
...
LSTM *make_lstm(Weights *weights, int batch_size, int input_size,
                int hidden_size) {
...
void lstm(LSTM *layer, float *input) {
  _lstm(input, layer->state_h, layer->state_c, layer->weights_input,
        ...
        layer->buffer, layer->batch_size, layer->input_size,
        layer->hidden_size);
}
...
Embedding *make_embedding(Weights *weights, int batch_size, int num_embeddings,
                          int embedding_dim) {
  size_t output_size = batch_size * embedding_dim * sizeof(float);
  Embedding *layer =
      (Embedding *)calloc(1, sizeof(Embedding) + batch_size + output_size);
  *layer = (Embedding){
      .output = (float *)(layer + 1),
...
void embedding(Embedding *layer, int *input) {
  _embedding(input, layer->weights, layer->output, layer->batch_size,
...
void one_hot(OneHot *layer, int *input) {
  _one_hot(input, layer->output, layer->batch_size, layer->input_size,
           layer->num_classes);
}
...
void cat_dim1(CatDim1 *layer, float *x, float *y) {
  _cat_dim1(x, y, layer->output, layer->batch_size, layer->x_size,
            layer->y_size);
}
...
Multidiscrete *make_multidiscrete(int batch_size, int logit_sizes[],
                                  int num_actions) {
...
void argmax_multidiscrete(Multidiscrete *layer, float *input, int *output) {
  _argmax_multidiscrete(input, output, layer->batch_size, layer->logit_sizes,
                        layer->num_actions);
}

void softmax_multidiscrete(Multidiscrete *layer, float *input, int *output) {
  _softmax_multidiscrete(input, output, layer->batch_size, layer->logit_sizes,
                         layer->num_actions);
}
...
Default *make_default(Weights *weights, int num_agents, int input_dim,
                      int hidden_dim, int action_dim) {
...
ConvLSTM *make_convlstm(Weights *weights, int num_agents, int input_dim,
                        int input_channels, int cnn_channels, int hidden_dim,
                        int action_dim) {
  ConvLSTM *net = (ConvLSTM *)calloc(1, sizeof(ConvLSTM));
  net->num_agents = num_agents;
  net->obs = (float *)calloc(num_agents * input_dim * input_dim * input_channels,
                    sizeof(float));
  net->conv1 = make_conv2d(weights, num_agents, input_dim, input_dim,
                           input_channels, cnn_channels, 5, 3);
  net->relu1 = make_relu(num_agents, hidden_dim * 3 * 3);
  net->conv2 =
      make_conv2d(weights, num_agents, 3, 3, cnn_channels, cnn_channels, 3, 1);
