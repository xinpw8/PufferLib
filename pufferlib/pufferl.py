    ...
    def evaluate(self, count_steps: bool = True):
        ...
        while self.full_rows < self.segments:
            ...
            with torch.no_grad(), self.amp_context:
                ...
                # Debug: Check what action the model would choose
                if epoch % 10 == 0 and self.full_rows == 0:
                    with torch.no_grad():
                        probs = torch.softmax(logits[0], dim=-1)
                        max_prob, max_action = torch.max(probs, dim=-1)
                        print(f"[EVAL DEBUG] Epoch {epoch}: Max prob action={max_action.item()}, prob={max_prob.item():.6f}")
                        # Check if this is action 364 (c1c8)
                ...
                if epoch >= 5:  # After initial training for regular RL
                    with torch.no_grad():
                        ...
                        # Use deterministic selection if max probability > 0.99
                        use_deterministic = max_probs > 0.99
                        
                        if use_deterministic.any():
                            # Use argmax for high-confidence actions
                            deterministic_actions = torch.argmax(logits, dim=-1)
                            action, logprob, _ = pufferlib.pytorch.sample_logits(logits)
                            # Replace sampled actions with deterministic ones where confidence is high
                            action = torch.where(use_deterministic, deterministic_actions, action)
                            
                            if epoch % 10 == 0 and self.full_rows == 0:
                                print(f"[EVAL DEBUG] Using deterministic action selection for {use_deterministic.sum().item()} envs")
                        else:
                            action, logprob, _ = pufferlib.pytorch.sample_logits(logits)
                else:
                    action, logprob, _ = pufferlib.pytorch.sample_logits(logits)
                
                # Debug: Check what action was actually sampled
                if epoch % 10 == 0 and self.full_rows == 0:
                    print(f"[EVAL DEBUG] Final action={action[0].item() if action.numel() > 0 else 'EMPTY'}")
...
def sweep(args=None, env_name=None):
    ...
    for i in range(args['max_runs']):
        ...
        try:
            ...
            print(f"Skipping run {i+1}/{args['max_runs']} due to APIUsageError: {e}")
            continue
        except Exception as e:
            print(f"Skipping run {i+1}/{args['max_runs']} due to unexpected error: {e}")
            continue
