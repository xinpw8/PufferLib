...
static double puf_dict_get_or(Dict* dict, const char* key, double fallback) {
...
static const char* puf_reset(void) {
...
static void puf_panel_header(const char* eval_t, const char* eval_pct) {
    ...
    printf("%s %-9s %13s%s    %s%-12s%s %s%6s %4s%s    %s%-10s %7s%s    ",
        puf_cyan(), "Summary", "Value", puf_reset(),
        puf_bcyan(), "Evaluate", puf_reset(), puf_bwhite(), eval_t, eval_pct, puf_reset(),
        puf_cyan(), "Losses", "Value", puf_reset());
    printf("%s│%s", puf_bcyan(), puf_reset());
    puf_dashboard_eol();
}
...
static void puf_panel_row(const char* s_name, const char* s_val,
    ...
    printf("%s %s%-9s%s %s%13s%s    %s%-12s%s %s%6s %4s%s    %s%-10s %7s%s    ",
        puf_reset(),
        puf_white(), s_name, puf_reset(), puf_bwhite(), s_val, puf_reset(),
        perf_color, p_name, puf_reset(), puf_bwhite(), p_time, p_pct, puf_reset(),
        puf_bwhite(), l_name, l_val, puf_reset());
    printf("%s│%s", puf_bcyan(), puf_reset());
    puf_dashboard_eol();
}
...
static void puf_user_header(void) {
    ...
    printf("%s %-23s %9s%s   %s%-23s %9s%s        ",
        puf_cyan(), "User Stats", "Value", puf_reset(),
        puf_cyan(), "User Stats", "Value", puf_reset());
    printf("%s│%s", puf_bcyan(), puf_reset());
    puf_dashboard_eol();
}
...
static void puf_user_row(const char* left_key, double left_val,
    ...
    if (has_right) {
        printf("%s %s%-23s %9.3f%s   %s%-23s %9.3f%s        ",
            puf_reset(),
            puf_bwhite(), left_key, left_val, puf_reset(),
            puf_bwhite(), right_key, right_val, puf_reset());
    } else {
...
static void puf_dashboard_blank(void) {
    printf("%s│%*s│%s", puf_bcyan(), PUF_DASH_WIDTH - 2, "", puf_reset());
    puf_dashboard_eol();
}
...
static void puf_dashboard_print(Dict* cfg, PuffeRL* p, Dict* log, int epoch) {
    ...
    puf_duration(uptime, sizeof(uptime), puf_dict_get_or(log, "uptime", 0));
    ...
    printf("%s %sPufferLib %s4.0%s        %s🐡%s        %sGPU:%s %2.0f%%    %sVRAM:%s %.1f/%.0fG    %sRAM:%s %.1fG        ",
        puf_reset(),
        puf_bcyan(), puf_bwhite(), puf_reset(),
        puf_bcyan(), puf_reset(),
        puf_cyan(), puf_bwhite(),
        puf_dict_get_or(log, "util/gpu_percent", 0),
        puf_cyan(), puf_bwhite(),
        puf_dict_get_or(log, "util/vram_used_gb", 0),
        puf_dict_get_or(log, "util/vram_total_gb", 0),
        puf_cyan(), puf_bwhite(),
        puf_dict_get_or(log, "util/cpu_mem_gb", 0));
    printf("%s│%s", puf_bcyan(), puf_reset());
    puf_dashboard_eol();
    puf_dashboard_blank();
    ...
    puf_perf_value(gpu_t, sizeof(gpu_t), gpu_pct, sizeof(gpu_pct),
        puf_dict_get_or(log, "perf/eval_gpu", 0), perf_total);
    puf_perf_value(env_t, sizeof(env_t), env_pct, sizeof(env_pct),
        puf_dict_get_or(log, "perf/eval_env", 0), perf_total);
    ...
    puf_perf_value(misc_t, sizeof(misc_t), misc_pct, sizeof(misc_pct),
        puf_dict_get_or(log, "perf/train_misc", 0), perf_total);
    puf_perf_value(forward_t, sizeof(forward_t), forward_pct, sizeof(forward_pct),
        puf_dict_get_or(log, "perf/train_forward", 0), perf_total);
