#ifndef PUFFER_VCACHE_H
#define PUFFER_VCACHE_H
...
static float pva_half_to_float(uint16_t h) {
...
static int pva_read(void* dst, size_t size, FILE* file) {
    return fread(dst, 1, size, file) == size;
}

static int pva_read_u32(FILE* file, uint32_t* out) {
    return pva_read(out, sizeof(*out), file);
}

static int pva_read_i32(FILE* file, int32_t* out) {
    return pva_read(out, sizeof(*out), file);
}

static int pva_read_f32(FILE* file, float* out) {
    return pva_read(out, sizeof(*out), file);
}

static void pva_set_mesh_frame(Mesh* mesh, PvaMeshClip* clip, int frame, int update_gpu) {
...
static void pva_unload_clip(PvaClip* clip) {
    if (clip == NULL) return;
...
static PvaClip* pva_load_clip(const char* path) {
    ...
    clip->model.transform = MatrixIdentity();
    ...
    for (uint32_t i = 0; i < material_count; i++) {
        ...
        if ((flags & PVA_FLAG_COLORS) != 0) {
            ...
            clip->model.materials[i].maps[MATERIAL_MAP_DIFFUSE].color = (Color){
                (unsigned char)(Clamp(color[0], 0.0f, 1.0f) * 255.0f),
                (unsigned char)(Clamp(color[1], 0.0f, 1.0f) * 255.0f),
                (unsigned char)(Clamp(color[2], 0.0f, 1.0f) * 255.0f),
                (unsigned char)(Clamp(color[3], 0.0f, 1.0f) * 255.0f),
            };
        }
    }
...
static void pva_update_clip(PvaClip* clip, int frame) {
    if (clip == NULL || clip->frame_count == 0) return;
    frame %= clip->frame_count;
    if (frame < 0) frame += clip->frame_count;
...
static int pva_frame_at_time(PvaClip* clip, float seconds) {
    if (clip == NULL || clip->frame_count == 0 || clip->fps <= 0.0f) return 0;
